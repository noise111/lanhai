...
if ($do == 'display') {
	...
	if((count($list))==1){
		header('Location: ./index.php?c=account&a=display&do=switch&uniacid=1&type=1');
		exit;
	}
...
if ($do == 'switch') {
	...
	if (!empty($uniacid)) {
		...
		if ($account_info['endtime'] > 0 && TIMESTAMP > $account_info['endtime']) {
			itoast('帐号已到期。', '', 'error');
		}
		...
		if (in_array($type, array(ACCOUNT_TYPE_APP_NORMAL, ACCOUNT_TYPE_APP_AUTH, ACCOUNT_TYPE_PHONEAPP_NORMAL, ACCOUNT_TYPE_ALIAPP_NORMAL))) {
			...
			if (!empty($module_name) && !empty($version_id)) {
				...
				if (!empty($module_info['account']['uniacid'])) {
					$uniacid = $module_info['account']['uniacid'];
				} else {
