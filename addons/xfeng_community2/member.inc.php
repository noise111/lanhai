 ...
 */
global $_GPC, $_W;
$ops = array('list', 'detail', 'add', 'update', 'family', 'create', 'detail', 'home', 'userinfo');
$op = trim($_GPC['op']);
if (!in_array($op, $ops)) {
...
elseif ($op == 'update') {
    ...
    if ($license) {
        $car = pdo_get('xcommunity_xqcars', array('car_num' => $member['license'], 'addressid' => $addressid), array());
        if (empty($car)) {
            $card = array(
                'uniacid' => $_W['uniacid'],
                'regionid' => $regionid,
        ...
        pdo_update('xcommunity_member', array('license' => $license), array('id' => $member['id']));
        $data = array();
        $data['content'] = '修改成功';
        util::send_result($data);
        exit();
    ...
    if (pdo_insert('xcommunity_member_log', $d)) {
        ...
        if ($logid) {
            ...
            if (pdo_insert('xcommunity_member_family', $dat)) {
                ...
                $img = "../addons/" . $this->module['name'] . "/template/family_qr/qrcode_{$_W['uniacid']}_{$logid}.png";
                $imgHtml = createQr($url, $img);
                $data = array();
...
//    $regionid = 1;
    ...
    if (!empty($category)) {
        ...
        foreach ($category as $cid => $cate) {
            $tsql = "select t1.id,t1.title,t1.url,t1.thumb,t1.show from" . tablename('xcommunity_housecenter') . "as t1 left join " . tablename('xcommunity_housecenter_region') . "as t2 on t1.id=t2.nid where t1.uniacid=:uniacid and t2.regionid=:regionid and t1.status = 1 and t1.pcate =:pcate order by t1.displayorder asc,t1.id asc ";
            ...
            if ($children) {
                foreach ($children as $key => $val) {
                    ...
                    if($val['show']==1){
                        ...
                        $data['total'] = model_shop::getOrderTotal($status,$regionid,$_W['member']['uid']);
                    }
    ...
    $data['license'] = $_SESSION['coummunity']['license'];
