 ...
 */
global $_GPC,$_W;
$op = in_array(trim($_GPC['op']),array('add','list','edit')) ? $_GPC['op'] : 'list';
$id = intval($_GPC['id']);
echo 3333;exit();
if ($op == 'list') {
    ...
    include $this->template('web/group/list');
}elseif ($op == 'edit') {
    ...
    include $this->template('web/group/edit');
}elseif ($op == 'add') {
    $id = intval($_GPC['id']);
    if (!empty($id)) {
    ...
    include $this->template('web/group/add');
}