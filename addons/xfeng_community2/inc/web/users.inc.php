...
//	$uuid = intval($_GPC['uuid']);
    ...
    if(checksubmit()) {
        ...
        if ($uid) {
            ...
            $insert = array(
                ...
                'permission' => $this->module['name'].'_menu_manage'
            );
	...
	if (checksubmit('submit')) {
		...
		if ($id) {
			...
			if ($r) {
				...
				if (empty($result)) {
					$url = "c=home&a=welcome&do=ext&m=$this->module['name']";
    ...
    if (checksubmit('submit')) {
        if($_GPC['cash']<= 0){
            message('输入金额不正确,请重新输入',referer(),'error',true);exit();
        }
        if ($_GPC['cash'] > $users['balance']) {
            message('余额不足，无法提现', referer(), 'error',true);
        }
        ...
        if ($r) {
            ...
            message('提交成功', referer(), 'success',true);
        }
    }
    ...
    include $this->template('web/users/m');
}
