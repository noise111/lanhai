 ...
 */
global $_W, $_GPC;
$ops = array('list', 'detail','add','display','appdel','applist');
$op = trim($_GPC['op']);
if (!in_array($op, $ops)) {
...
elseif($op == 'applist'){
    $list = pdo_fetchall("select t1.id,t1.place_num,t1.regionid from" . tablename('xcommunity_parking') . "t1 where t1.uniacid=:uniacid",array(':uniacid' => $_W['uniacid']));
    if (empty($list)) {
        util::send_error(-1, '参数错误');
    }
    util::send_result($list);
