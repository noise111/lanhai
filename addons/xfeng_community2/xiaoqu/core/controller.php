...
class Controller
{
	public function template($name = '')
	{
		...
		$compile = IA_ROOT . '/data/tpl/web/xfeng_community_web/' . ES_STYLE . '/' . $name . '.tpl.php';
	...
	public function itoast($msg, $redirect = '', $type = '')
	{
		global $_W;
		global $_GPC;
		...
		if ($_W['isajax'] || !empty($_GET['isajax']) || ($type == 'ajax')) {
			if (($type != 'ajax') && !empty($_GPC['target'])) {
				exit("\n" . '<script type="text/javascript">' . "\n" . 'parent.require([\'jquery\', \'util\'], function($, util){' . "\n\t" . 'var url = ' . ((!empty($redirect) ? 'parent.location.href' : '\'\'')) . ';' . "\n\t" . 'var modalobj = util.itoast(\'' . $msg . '\', \'\', \'' . $type . '\');' . "\n\t" . 'if (url) {' . "\n\t\t" . 'modalobj.on(\'hide.bs.modal\', function(){$(\'.modal\').each(function(){if($(this).attr(\'id\') != \'modal-itoast\') {$(this).modal(\'hide\');}});top.location.reload()});' . "\n\t" . '}' . "\n" . '});' . "\n" . '</script>');
			}
			 else {
				$vars = array();
				$vars['itoast'] = $msg;
		...
		include $this->template('common/itoast');
		exit();
	}
}
