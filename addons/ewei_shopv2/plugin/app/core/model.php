...
if( !class_exists("AppModel") ) 
{
    ...
    /**
		...
		public function getTMessage($id) 
		{
			...
			$item = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_wxapp_tmessage") . " WHERE $id_where AND uniacid=:uniacid AND status=1 LIMIT 1", array(":uniacid" => $_W["uniacid"] ));
			if( !empty($item) ) 
			{
...
//			}
        ...
        public function getMerchInfo($openid){
            ...
            $id_where = '';
            if($id > 0){
                $id_where = " a.id = '$openid' ";
            } else {
                $id_where = " (a.openid = '$openid' OR u.openid = '$openid') ";
            }
            $sql = "SELECT a.openid, a.merchid, a.username, a.status, a.isfounder, a.roleid,
                    u.openid as user_openid, u.groupid, u.merchname, u.salecate, u.realname as u_realname, u.mobile as u_mobile, u.benefitbalance as u_benefitbalance, u.status as user_status " . 
                   "FROM " . tablename("ewei_shop_merch_account") . " a " .
                   "LEFT JOIN " . tablename("ewei_shop_merch_user") . " u ON a.merchid = u.id " .
                   "WHERE $id_where AND a.status = 1 AND u.status = 1";
            $info = pdo_fetch($sql);
            return $info;
        }    
