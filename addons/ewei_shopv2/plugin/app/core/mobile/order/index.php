...
//        app_json(array( "list" => $list ));
		...
		foreach( $list as &$row ) 
		{
			...
			$sql = "SELECT og.goodsid,og.total,g.title,g.thumb,g.type, og.price,og.optionname as optiontitle,og.optionid,op.specs,
                    g.merchid,g.status FROM " . 
	...
	public function detail() 
	{
		...
		$gn = 0;
		$nog = 0;
		$goods = pdo_fetchall("select g.id,g.type, og.goodsid,og.price,og.oldprice, g.title,g.thumb,g.status,og.total,g.credit,
                og.optionid,og.optionname as optiontitle,g.isverify,g.isfullback,g.storeids, og.onlinehomeget, og.couponid, 
                g.originbarcode, g.couponbarcode, g.barcodecoupontype, g.verifybarcode ".
                $diyformfields . 
		...
		$order = array( 
            ...
            "refundtext_btn" => "", "cancancel" => !$order["userdeleted"] && !$order["status"], 
		...
		foreach( $goods as &$g ) 
		{
            ...
            $barcode_type = 1; //原价
            if($g['couponid']){
                $coupon = com('coupon')->getCouponByDataID($g['couponid']);
                if($coupon && $coupon['coupontype'] == $g['barcodecoupontype']){
                    $barcode_type = 2; //优惠券价
                }
            }
            $barcode = "";
            switch ($barcode_type){
                case 1:
                    $barcode = $g['originbarcode'];
                    break;
                case 2:
                    $barcode = $g['couponbarcode'];
                    break;
                default : 
                    $barcode = $g['originbarcode'];
                    break;
			...
			$allgoods[] = array( 
                ...
                "barcode" => $barcode,
                "oldprice"  => $g['oldprice'],
                "couponprice" => $g['oldprice'] - $g['price']
            );
		}
		unset($g);
		if( !empty($allgoods) ) 
        ...
        //门店信息
        if($order['storeid'] || $order['verifystoreid']){
            $sid = empty($order['storeid']) ? $order['verifystoreid'] : $order['storeid'];
            $store_info = m('store')->getStoreInfo($sid);
            $result['storename'] = $store_info['storename'];           
        } else {
            $result['storename'] = "";
        }
        //结算信息
        $order_benefit = pdo_get("ewei_shop_order_benefit_log", array("orderid" => $order["id"]));
        if($order_benefit){
            $result['storebenefit'] = $order_benefit['storebenefit'];
        } else {
            $result['storebenefit'] = 0;
        }
        //利润信息
        $result['profit'] = 0;
        foreach ($goods as $good){
            $result['profit'] += floatval($good['onlinehomeget']);
        }
        //导购信息
        if($order_benefit['saleropenid']){
            $saler = m('store')->getSalerInfo(array('openid' => $order_benefit['saleropenid']));
            $result['saler_nickname'] = $saler['salername'];
            $result['saler_avatar'] = $saler['avatar'];
            $result['saler_mobile'] = $saler['mobile'];
        } else {
            $result['saler_nickname'] = "";
            $result['saler_avatar'] = "";
            $result['saler_mobile'] = "";
        }
