...
class Create_EweiShopV2Page extends AppMobilePage 
{
	...
	public function main() 
	{
		...
		if( !$packageid ) 
		{
			...
			if( empty($id) ) 
			{
				...
				else 
				{
					$sql = "SELECT c.goodsid,c.total,g.maxbuy,g.type,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype" .
                           ...
                           " g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale,g.depositpercent,g.depositprice " .
			...
			{
				$carrier_list = pdo_fetchall("select * from " . tablename("ewei_shop_store") . " where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(1,3) order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"], ":merchid" => $merch_id ));
			}
			...
			foreach( $goods as &$g ) 
			{
				...
				else 
				{
					$gprice = $g["marketprice"] * $g["total"];
					//计算定金
					$gdepositprice = $g["depositprice"];
//					$gdepositpercent = $g["depositpercent"] * $gprice/100;
					$gdepositpercent = $g["depositpercent"];
                ...
                $goodsdepositprice += $gdepositprice;
                $goodsdepositpercent += $gdepositpercent;
				$total += $g["total"];
				if( empty($bargain_id) ) 
				{
					if( $g["seckillinfo"] && $g["seckillinfo"]["status"] == 0 ) 
...
//            app_json(array('list'=>666));
		}
		else 
		{
//		    app_json(array('list'=>777));
			$merchdata = $this->merchData();
			extract($merchdata);
			$merch_array = array( );
			$merchs = array( );
			...
			$goods = array( );
			$goodsprice = 0;
			$goodsdepositpercent = 0;
			$goodsdepositprice = 0;
			$marketprice = 0;
			$goods_list = array( );
			foreach( $g as $key => $value ) 
			{
				$goods[$key] = pdo_fetch("select id,title,thumb,marketprice,merchid,dispatchtype,dispatchid,dispatchprice,depositprice,depositpercent from " . tablename("ewei_shop_goods") . "\r\n                            where id = " . $value["goodsid"] . " and uniacid = " . $uniacid . " ");
				if( $is_openmerch == 1 ) 
				{
                ...
                $goodsdepositpercent += price_format($goods[$key]["goodsdepositpercent"]);
                $goodsdepositprice += price_format($goods[$key]["goodsdepositprcie"]);
			}
//            app_json(array('list'=>$g));
			$address = pdo_fetch("select * from " . tablename("ewei_shop_member_address") . " where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1", array( ":uniacid" => $uniacid, ":openid" => $openid ));
			$total = count($goods);
		...
		if( is_array($goodsarr) ) 
		{
			...
			else 
			{
				if( 0 < $discount && $backtype == 1 ) 
				{
					...
					if( 0 < $merchid ) 
					{
						$coupondeduct_text = "平台优惠券折扣(" . $discount . "折)";
					}
					else 
					{
...
//		$isdeposit = intval($_GPC['isdeposit']);//是否支付定金；1：支付定金；0：支付全款
		$dflag = $_GPC["dflag"];
		...
		if( $couponid ) 
		{
            foreach($couponid as $key => $cid){
                if(isset($allgoods[$key])){
                    $coupon_data = $this->saler_calculate_coupon($cid, $allgoods[$key]);
                    $coupon_deductprice += $coupon_data['deductprice'];
                }
            }
        } 
		$gifts = array( );
...
//		$return_array["deposits"] = $dispatch_price + $seckill_dispatchprice;
...
//                   " buyagain,buyagain_islong,buyagain_condition, buyagain_sale,verifygoodsdays,verifygoodslimittype,verifygoodslimitdate, " .
//                   " onlinestoreget, onlinehomeget, storeget, salerfirst, salermore, agentonline, agenthome, teambenefit " .
...
//                
//                //新增分润功能
//                $order_goods["onlinestoreget"] = $goods["onlinestoreget"];
//                $order_goods["onlinehomeget"] = $goods["onlinehomeget"];
//                $order_goods["storeget"] = $goods["storeget"];
//                $order_goods["salerfirst"] = $goods["salerfirst"];
//                $order_goods["salermore"] = $goods["salermore"];
//                $order_goods["agentonline"] = $goods["agentonline"];
//                $order_goods["agenthome"] = $goods["agenthome"];
//                $order_goods["teambenefit"] = $goods["teambenefit"];
//                
...
//			}
//            
//            $goods_coupon = $_GPC['goods_coupon'];
//			foreach( $allgoods as $goods ) 
//			{
...
//                
//                //新增分润功能
//                if($goods_coupon && isset($goods_coupon[$goodsid])){
//                    $order_goods['couponid'] = $goods_coupon[$goodsid];
//                }
//                $order_goods["onlinestoreget"] = $goods["onlinestoreget"];
//                $order_goods["onlinehomeget"] = $goods["onlinehomeget"];
//                $order_goods["storeget"] = $goods["storeget"];
...
//			}
//		}
//        //根据快递地址信息将订单划分给对应的运营中心
//        if($addressid){
//            $sql = "SELECT m.id FROM " . tablename("ewei_shop_member") . " m , " . tablename("ewei_shop_member_address") . " a " .
//                   " WHERE a.id = $addressid AND m.isaagent = 1 AND m.aagentstatus = 1" .
...
//        m('notice')->sendOrderCreateMessage($orderid);
...
//	}
    ...
    public function submit() 
	{
		...
		$openid = $_W["openid"];
		$uniacid = $_W["uniacid"];
//		app_json(array('sss' => $_GPC));
		//如果$_GPC['isdeposit'] 为真 则使用定金，未false不使用
		$member = m("member")->getMember($openid);
		if( $member["isblack"] == 1 ) 
		{
			app_error(AppError::$UserIsBlack);
		...
		foreach( $goods as $g ) 
		{
			...
			$sql = "SELECT id as goodsid,title,type, weight,total,issendfree,isnodiscount, thumb,marketprice,cash,isverify,isforceverifystore,verifytype," . 
                   ...
                   " isdiscount,isdiscount_time,isdiscount_discounts, virtualsend," .
                   " buyagain,buyagain_islong,buyagain_condition, buyagain_sale,verifygoodsdays,verifygoodslimittype,verifygoodslimitdate, " .
                   " onlinestoreget, onlinehomeget, storeget, salerfirst, salermore, agentonline, agenthome, teambenefit,depositprice,depositpercent " .
                   " FROM " . tablename("ewei_shop_goods") . 
                   " where id=:id and uniacid=:uniacid  limit 1";
			$data = pdo_fetch($sql, array( ":uniacid" => $uniacid, ":id" => $goodsid ));
			$data["seckillinfo"] = plugin_run("seckill::getSeckill", $goodsid, $optionid, true, $_W["openid"]);
        ...
        //分配优惠券id
        $deposits = 0;
//        app_json(array('list'=>$_GPC['isdeposit'],'sss'=>$_GPC));die;
        foreach($allgoods as $key => &$g){
            if(isset($couponid[$key]) && !empty($couponid[$key])){
                $coupon_return = $this->saler_calculate_coupon($couponid[$key], $g);
                $totalprice += $coupon_return['couponprice'];
                $goodsprice += $coupon_return['marketprice'];
                $couponprice += $coupon_return['deductprice'];
                $g['couponid'] = $couponid[$key];
            } else {
                $totalprice += $g['marketprice'];
                $goodsprice += $g['marketprice'];
                $couponprice += 0;
                $g['couponid'] = 0;
            }
            //计算定金
            if($_GPC['isdeposit'] == "true"){
                if($g['depositprice'] > 0 || $g['depositpercent'] > 0){
                    if($g['depositprice'] > 0){
                        $deposits += $g['depositprice'];
                    }
                    //注释取消定金类型
//                    else{
//                        $deposits += $g['marketprice']*$g['depositpercent']/100;
////                        $deposits += $g['depositpercent'];
////                    $deposits += 1;
//                    }
                }else{
                }
...
//        app_json(array('list'=>$allgoods,"aaaa"=>$deposits));die;
		if( $isonlyverifygoods ) 
		{
			$addressid = 0;
		}
...
//        app_json(array( "order" => $order ));
        ...
        foreach( $allgoods as $goods ) 
        {
            $goods_coupon = $this->saler_calculate_coupon($goods['couponid'], $goods);
            ...
            $order_goods["goodssn"] = $goods["goodssn"];
            $order_goods["productsn"] = $goods["productsn"];
            $order_goods["realprice"] = $goods_coupon['couponprice']; //折后价
            $order_goods["oldprice"] = $goods_coupon['marketprice']; //原价
            $order_goods['couponid'] = $goods['couponid'];
     ...
     * 导购扫码计算购物车商品价格
     */
    public function saler_caculate($is_submit = false){
        global $_W;
        global $_GPC;
        $uniacid = $_W['uniacid'];
        $openid = $_W['openid'];
        ...
        $allgoods = array();
        foreach($goods_arr as $cart_goods){
            $cartid = $cart_goods['id'];
            $goodsid = $cart_goods['goodsid'];
            $sql = "SELECT g.id as goodsid, g.title, g.marketprice, c.couponid, c.id, c.total
                    FROM " . tablename('ewei_shop_goods') . " g ".
                   " INNER JOIN " . tablename('ewei_shop_member_salercart') . " c on c.goodsid = g.id AND c.openid = '$openid'" .
                   " WHERE c.uniacid = $uniacid AND c.id = $cartid LIMIT 1";
        ...
        foreach($allgoods as $goods){
            $coupon_price = $this->saler_calculate_coupon($goods['couponid'], $goods);
            $totalprice  += $coupon_price['couponprice'];
            $deductprice += $coupon_price['deductprice'];
            $originprice += $coupon_price['marketprice'];
            $total       += $goods['total'];
        }
        $return_array = array(
    ...
    public function saler_calculate_coupon($couponid, $goods_data){
        global $_W;
        global $_GPC;
        ...
        $return = array(
            ...
            'couponprice'       => $goods_data['marketprice']
        );
        if(!$couponid){
            return $return;
        }
        ...
        $sql = "SELECT d.id,d.couponid,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodids,c.limitgoodcateids,c.limitdiscounttype  FROM " . tablename("ewei_shop_coupon_data") . " d";
		$sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
		$sql .= " where d.id=:id and d.uniacid=:uniacid and d.used=0 limit 1";
        $coupon_data = pdo_fetch($sql, $params);
        if($coupon_data){
            if($coupon_data['deduct'] > 0){
                $return['deductprice'] = $coupon_data['deduct'];
                //优惠券立减价少于市场价
                if($return['marketprice'] >= $coupon_data['deduct']){
                    $return['couponprice'] = $return['marketprice'] - $coupon_data['deduct'];
                } else {
                    $return['deductprice'] = $return['marketprice'];
    ...
    public function saler_submit(){
        ...
        //获取购物车中的数据
        $sql = " SELECT s.*, g.marketprice, g.title,
                 g.onlinestoreget, g.onlinehomeget, g.storeget, g.salerfirst, g.salermore, g.agentonline, g.agenthome, g.teambenefit
               ...
               " INNER JOIN " . tablename('ewei_shop_goods') . " g ON g.id = s.goodsid AND g.uniacid = :uniacid
                 WHERE s.openid = :openid AND s.uniacid = :uniacid AND s.deleted = 0 AND s.selected = 1";
        $cart_goods = pdo_fetchall($sql, $params);
        if(!$cart_goods || empty($cart_goods)){
            app_error(AppError::$OrderCreateNoGoods);
        }
        //获取店员数据
        $saleropenid = $cart_goods[0]['saleropenid'];
        $sql = " SELECT * FROM " . tablename('ewei_shop_saler') . " WHERE uniacid = $uniacid AND openid = '$saleropenid' LIMIT 1";
        $saler = pdo_fetch($sql);
        ...
        $now = time();
        //生成order
        $_GPC['goods'] = $cart_goods;
        $calculate_price = $this->saler_caculate(true);
        $order = array();
        $ordersn = m("common")->createNO("order", "ordersn", "SH");
        $order['uniacid']       = $uniacid; 
        $order['openid']        = $openid;
        $order['ordersn']       = $ordersn;
        $order['price']         = $calculate_price['totalprice'];
        $order['goodsprice']    = $calculate_price['totalprice'];
        //$order['deductprice']   = $calculate_price['deductprice'];
        $order['couponprice']   = $calculate_price['deductprice'];
        $order['createtime']    = $now;
        $order['isverify']      = 1;
        $order['storeid']       = $saler['storeid'];
        $order['merchid']       = $saler['merchid'];
        $order['multicoupon']   = 1;
        $order['saleropenid']   = $cart_goods[0]['saleropenid'];
        $order['status']        = 0;
        if($paytype){
        ...
        if(!$orderid){
            ...
            if($paytype == 4){
                m("notice")->sendSalerOrderBenefitMessage($orderid);
            }
        }
        foreach($cart_goods as $goods){
            ...
            $goods_data = array(
                ...
                'openid'        => $openid,
                'goodsid'       => $goods['goodsid'],
                'price'         => $goods_coupon['couponprice'],
                'total'         => $goods['total'],
                'title'         => $goods['title'],
                'createtime'    => $now,
                'realprice'     => $goods_coupon['couponprice'], //折后价
                'oldprice'      => $goods_coupon['marketprice'], //原价
                'couponid'      => $goods['couponid'],
                'onlinestoreget'    => $goods['onlinestoreget'],
                'onlinehomeget'     => $goods['onlinehomeget'],
                'storeget'      => $goods['storeget'],
                'salerfirst'    => $goods['salerfirst'],
                'salermore'     => $goods['salermore'],
                'agentonline'   => $goods['agentonline'],
                'agenthome'     => $goods['agenthome'],
                'teambenefit'   => $goods['teambenefit'],
                'qrcodeid'      => $goods['qrcodeid'],
                'qrcodeno'      => $goods['qrcodeno']
            );
            pdo_insert("ewei_shop_order_goods", $goods_data);
            
            //更新对应的qrcode log
            if($goods['qrcodeid'] && $goods['qrcodeno']){
                pdo_update('ewei_shop_goods_qrcode_log', array('openid' => $openid), array('uniacid' => $uniacid, 'qid' => $goods['qrcodeid'], 'no' => $goods['qrcodeno']));
            }
        }
        
        //删除购物车
        pdo_query("DELETE FROM " . tablename("ewei_shop_member_salercart") . " WHERE  openid=:openid and uniacid=:uniacid and selected=1 ", array( ":uniacid" => $uniacid, ":openid" => $openid ));
        //更新优惠券状态
        $plugincoupon = com("coupon");
        $plugincoupon->useConsumeCoupon($orderid);
        
        //获取条码信息
        $barcode = $this->get_order_barcode($orderid);
        //获取订单商品信息
        $sql = " SELECT og.id, og.goodsid, og.total, og.price, g.title, g.marketprice, g.thumb
        ...
        foreach($order_goods as &$og){
            $og['barcode'] = $barcode[$og['id']];
        }
        $order_goods = set_medias($order_goods, 'thumb');
        $order['total'] = count($order_goods);
        $order['originprice'] = $calculate_price['originprice'];
        m('notice')->sendOrderCreateMessage($orderid);
        app_json(array('id' => $orderid, 'list' => $order_goods, 'order' => $order));
    }
    
    public function saler_qrcode($salerid = 0){ 
        global $_GPC;
        if(empty($salerid)){
        ...
        if($saler){
            $scene = "salerid=".$saler['id'];
            $content = com('appqrcode')->getQrcodeContent($scene);
     ...
     * 获取订单条形码信息
     */
    public function get_order_barcode($orderid = 0){
        global $_W;
        global $_GPC;
        $uniacid = $_W['uniacid'];
        $openid = $_W['openid'];
        ...
        $sql = "SELECT id FROM " . tablename('ewei_shop_order') . " WHERE openid = :openid AND id = :orderid AND uniacid = :uniacid";
        $order = pdo_fetchcolumn($sql, array(':openid' => $openid, ':orderid' => $orderid, ':uniacid' => $uniacid));
        if($order){
            $barcode_list = array();
            $sql = " SELECT g.originbarcode, g.couponbarcode, g.barcodecoupontype, g.verifybarcode, og.couponid, og.id
                     FROM " . tablename('ewei_shop_order_goods') . ' og '.
                   " INNER JOIN " . tablename('ewei_shop_goods') . " g ON g.id = og.goodsid AND g.uniacid = :uniacid
                     WHERE og.orderid = :orderid";
            $order_goods = pdo_fetchall($sql, array(':orderid' => $orderid, ':uniacid' => $uniacid));          
            ...
            if($is_return){
                ...
                app_json(array('id' => $orderid, 'code_list' => $barcode_list));
            }
        } else {
