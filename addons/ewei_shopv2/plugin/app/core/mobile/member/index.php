...
class Index_EweiShopV2Page extends AppMobilePage 
{
    ...
    public function get_saler(){
        global $_W;    
        $sql = "SELECT saleropenid FROM " . tablename('ewei_shop_coupon_relation') . " WHERE customeropenid = '{$_W['openid']}' AND uniacid = {$_W['uniacid']}";
        $saleropenid = pdo_fetchcolumn($sql);
        if($saleropenid){
            $saler = m('store')->getSalerInfo(array('openid' => $saleropenid));
            if($saler['storeid']){
                $store = m('store')->getStoreInfo($saler['storeid']);
                if($store){
                    $saler['store_name']        = $store['storename'];
                    $saler['store_address']     = $store['address'];
                    $saler['store_logo']        = $store['logo'];
                    $saler['store_province']    = $store['province'];
                    $saler['store_city']        = $store['city'];
                    $saler['store_area']        = $store['area'];
                }
            }
            //获取导购服务次数
            $sql = "SELECT COUNT(id) FROM " . tablename("ewei_shop_order") .
                   " WHERE openid = '{$_W['openid']}' AND saleropenid = '{$saler['openid']}' AND uniacid = {$_W['uniacid']} AND status = 3";
    ...
    public function query(){
		...
		$list = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_member") . " WHERE 1 " . $condition . " order by id asc", $params);
