...
class Info_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		$memberArr = array(
            ...
            'benefitbalance' => $member['benefitbalance']
        );
		$result = array(
			'member'  => $memberArr,
        ...
        if($saler){
            ...
            $result['store'] = $store;
            if($store){
                $result['store_status'] = $store['status'];
            }  else {
                $result['store_status'] = 0;
            }
            $result['is_deliver'] = $saler['deliver'];
        } else {
            $result['is_saler'] = 0;
            $result['is_deliver'] = 0;
        }
        //获取判断是否是零售商
        $merch_info = m('member')->getMerchInfo($_W['openid']);
        $result['is_merch'] = !empty($merch_info) && $merch_info['is_merch'] == 1 ? 1 : 0 ; 
        $result['merch_list'] = $merch_info['list'];
        $result['merch_list'] = set_medias($result['merch_list'], "logo");
        
        //判断是否是运营中心
        $agent_info = m('member')->getAgentInfo($_W['openid']);    
        $result['is_agent'] = !empty($agent_info) &&  $agent_info['is_agent'] == 1 ? 1 : 0;
        $result['agent_list'] = $agent_info['list'];
        
        //获取优惠券数量
        $coupon_number = com("coupon")->getAvailableCount($_W['openid']);
        $result['coupon_number'] = $coupon_number ? $coupon_number : 0;
		app_json($result);
	}
