...
class Index_EweiShopV2Page extends AppMobilePage 
{
	public function main() 
	{
        ...
        if(is_array($page['data']['items'])){
            $cart_model = m('cart');
            foreach($page['data']['items'] as &$pitem){
                if($pitem['data'] && !empty($pitem['data']) && $pitem['id'] == 'goods'){
                    foreach($pitem['data'] as &$goods){
                        $coupon_data = $cart_model->getCouponsByGood($goods['gid'], array(COUPON_SOCIAL));
                        $goods['coupon_number'] = count($coupon_data);
                    }
                }
            }
        }
