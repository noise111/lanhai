...
class Goods_EweiShopV2Page extends AppMobilePage 
{
     ...
     * @return mixed
     */
    protected function check_member($openid){
        global $_W;
        global $_GPC;
        //是否是店员
        $params = array(
        ...
        $sql = "SELECT * FROM " . tablename('ewei_shop_saler') . " WHERE uniacid = :uniacid AND openid = :openid";
        
        $saler = pdo_fetch($sql, $params);
    ...
    public function scan(){
        ...
        global $_GPC;
        $member = $this->member;
        $saler = $this->check_member($member['openid']);
        app_json(array('saler' => $saler));
    }
}
?>