...
class Index_EweiShopV2Page extends AppMobilePage
{
    ...
    public function getdetail()
    {
        ...
        $detail = array( "name" => $coupon["couponname"], "merchstr" => ($coupon["merchname"] ? "限购" . $coupon["merchname"] . "平台商品" : ""), "money" => $coupon["money"], "credit" => $coupon["credit"], "title2" => $coupon["title2"], "title3" => $coupon["title3"], "color" => $coupon["color"], "gettypestr" => $coupon["gettypestr"], "coupontype" => $coupon["coupontype"], "limitdiscounttype" => $coupon["limitdiscounttype"], "limitgoodtype" => $coupon["limitgoodtype"], "limitgoodcatetype" => $coupon["limitgoodcatetype"], "limitgoods" => $goods, "limitcates" => $category, "islimitlevel" => $coupon["islimitlevel"] );
    ...
    public function set_share_coupon(){
        global $_W;
        global $_GPC;
        $sharebatch = trim($_GPC['sharebatch']);
        $openid = $_W['openid'];
        if(empty($openid)){
            app_error(AppError::$UserNotLogin);
        }
        //查找是否已领取过同类型的优惠券
        $sql = "SELECT id FROM " . tablename("ewei_shop_coupon_data") . " WHERE openid = '$openid' AND sender <> '$openid' AND sharebatch = '$sharebatch' AND usetime = 0 AND uniacid = {$_W['uniacid']}";
        $row = pdo_fetch($sql);
        if(empty($row)){
            $sql = "SELECT c.couponname, c.deduct, c.coupontype, cd.* FROM " . tablename('ewei_shop_coupon_data') . " cd ".
                   "LEFT JOIN " . tablename('ewei_shop_coupon') . " c ON c.id = cd.couponid " .
                   "WHERE cd.sharebatch = '$sharebatch' AND cd.uniacid = {$_W['uniacid']} AND c.coupontype = " . COUPON_SOCIAL . 
                   " AND cd.usetime = 0 AND cd.ordersn='' AND openid <> '$openid' ".
                   "ORDER BY cd.gettime asc";
            $info = pdo_fetch($sql);
            if($info){
                pdo_update('ewei_shop_coupon_data', array('openid' => $openid, 'gettime' => time()), array('id' => $info['id']));
                app_json(array('coupon' => $info));
            } else {
                app_error(AppError::$CouponNotFound);
            }
        } else {
    ...
    public function batch_detail(){
        ...
        $time = time();
        $select = "select d.id,d.couponid,d.gettime,d.used, d.sender,c.timelimit,c.coupontype,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,
        ...
        $sql .= " where (d.openid=:openid OR d.sender = :openid) and d.uniacid=:uniacid AND d.sharebatch = '$sharebatch' AND d.openid <> '$openid' ";
         if( !empty($past) ) {
        ...
        $sql .= " and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timeend>=" . $time . "))  ";
        
        $total = pdo_fetchcolumn("SELECT COUNT(d.id) " . $sql, array( ":openid" => $openid, ":uniacid" => $_W["uniacid"] ));
        $coupons = pdo_fetchall($select . $sql, array( ":openid" => $openid, ":uniacid" => $_W["uniacid"] ));
        foreach($coupons as &$coupon){
            $member = m('member')->getMemberBase($coupon['openid']);
            $coupon['member_nickname'] = $member['nickname'];
            $coupon['member_avatar'] = $member['avatar'];
        }
        app_json(array('list' => $coupons, 'total' => $total));
    }
    
    public function get_auto_goods_coupon(){
        global $_W;
        global $_GPC;
        $openid = $_W['openid'];
        //商品信息数组，键名为goodsid，值为数量
        ...
        $model = m("goods");
        $coupon_array = array();
        foreach($goodsids as $goodsid => $number){
            for($i = 1; $i <= $number; $i++){
                $couponid = $model->get_auto_goods_coupon($openid, $goodsid, $coupon_array);
                $coupon_array[] = $couponid ? $couponid : 0;
            }
        }
        app_json(array('couponid' => $coupon_array));
    }
}
