...
class My_EweiShopV2Page extends AppMobilePage
{
    public function getlist()
    {
        ...
        $psize = 5;
        $time = time();
        $select = "select d.id,d.couponid,d.gettime, d.sender,c.timelimit,c.coupontype,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,
                   c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.tagtitle,c.settitlecolor,
                   c.titlecolor, d.sharebatch, d.gettype, d.openid";
        $sql = " from " . tablename("ewei_shop_coupon_data") . " d";
        $sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
        
        $where = " where (d.openid=:openid OR d.sender = :openid) and d.uniacid=:uniacid ";
        if(!empty($past)) {
            $where .= " and  ( (c.timelimit =0 and c.timedays<>0 and  c.timedays*86400 + d.gettime <unix_timestamp()) or (c.timelimit=1 and c.timeend<unix_timestamp() ))";
        }
        else
        {
            if(!empty($used)) {
                ...
                $where .= " and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timeend>=" . $time . ")) and  d.used =0 ";
            }
        }
        ...
        pdo_update("ewei_shop_coupon_data", array( "isnew" => 0 ), $params);
        
        $total = pdo_fetchcolumn("SELECT COUNT(d.id) " . $sql . $where, $params);
        $batch_sql = $sql . $where . " group by d.sharebatch order by d.gettime desc  LIMIT " . ($pindex - 1) * $psize . "," . $psize;

        $coupon_sharebatchs = pdo_fetchall("SELECT d.sharebatch " . $batch_sql, $params, "sharebatch");
        $sharebatch_list = !empty($coupon_sharebatchs) ? array_keys($coupon_sharebatchs) : array();
        $sharebatch_list_str = implode(",", array_map(function($val){
            return sprintf("'%s'", $val);
        }, $sharebatch_list));
        if(!empty($sharebatch_list)){
            $list_sql = $select . $sql . $where . " AND d.sharebatch IN($sharebatch_list_str) order by d.gettime desc ";
            $coupons = set_medias(pdo_fetchall($list_sql, $params), "thumb");
        } else {
            $coupons = false; 
        }
        ...
        foreach( $coupons as $i => &$row ) 
        {
            ...
            if( $row["backtype"] == 0 ) 
            {
                ...
                if( $row["enough"] == "0" ) 
                {
                    if($row['coupontype'] == COUPON_SOCIAL){
                        $row["color"] = "red";
                        $tagtitle = "社交券";
                    } else {
                        $row["color"] = "org";
            ...
            if(!$row['sharebatch']){
                ...
                if(isset($return[$row['sharebatch']])){
                    $return[$row['sharebatch']][] = $row;
                } else {
                    $return[$row['sharebatch']] = array($row);
                }
            }
        ...
        sort($return);
        //获取券池分享详情
        $sql = "SELECT COUNT(id) as num, sharebatch FROM " . tablename("ewei_shop_coupon_data") . 
               " WHERE used = 0 AND openid = :openid AND sender <> openid 
                 AND sharebatch IN($sharebatch_list_str) " .
               " GROUP BY sharebatch";        
        $sharebatch_info = pdo_fetchall($sql, array(":openid" => $openid), "sharebatch");
        //券池统计数据，共有几张，已用几张
        $sql = "SELECT COUNT(id) as num, sharebatch FROM " . tablename("ewei_shop_coupon_data") . 
               " WHERE sender = :openid AND sharebatch IN($sharebatch_list_str) GROUP BY sharebatch";
        $sharebatch_total = pdo_fetchall($sql, array(":openid" => $openid), "sharebatch");
        $sql = "SELECT COUNT(id) as num, sharebatch FROM " . tablename("ewei_shop_coupon_data") . 
               " WHERE used <> 0 AND sender = :openid AND sharebatch IN($sharebatch_list_str) GROUP BY sharebatch";
        $sharebatch_used = pdo_fetchall($sql, array(":openid" => $openid), "sharebatch");
        app_json(array( 
            "list" => $return, 
            "pagesize" => $psize, 
            "list_count" => count($coupons), 
            "total" => $total, 
            "closecenter" => intval($set["closecenter"]),
            "sharebatch_info"   => $sharebatch_info ? $sharebatch_info : array(),
            "sharebatch_total"  => $sharebatch_total ? $sharebatch_total : array(),
            "sharebatch_used"   => $sharebatch_used ? $sharebatch_used : array()
        ));
    }
    ...
    public function getdetail()
    {
        ...
        $detail = array( "name" => $coupon["couponname"], "merchstr" => ($coupon["merchname"] ? "限购" . $coupon["merchname"] . "平台商品" : ""), "title2" => $coupon["title2"], "title3" => $coupon["title3"], "color" => $coupon["color"], "coupontype" => $coupon["coupontype"], "limitdiscounttype" => $coupon["limitdiscounttype"], "limitgoodtype" => $coupon["limitgoodtype"], "limitgoodcatetype" => $coupon["limitgoodcatetype"], "limitgoods" => $goods, "limitcates" => $category, "num" => $num );
        if( $coupon["timestr"] == "0" ) 
        {
            $detail["usestr"] = "永久有效";
        }
        ...
        if( empty($coupon["coupontype"]) || $coupon["coupontype"] == COUPON_SOCIAL ) 
        {
