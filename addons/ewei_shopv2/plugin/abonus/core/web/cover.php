...

{
...

	{
		...
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'ewei_shopv2区域运营中心入口设置'));
...

		{
...

			{
				...
				if ($keyword['name'] != 'ewei_shopv2区域运营中心入口设置') 
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2区域运营中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
			...
			plog('abonus.cover.edit', '修改区域运营中心入口设置');
