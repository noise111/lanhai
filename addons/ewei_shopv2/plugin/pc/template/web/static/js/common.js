$(function(){
	...
	var act = "store_list";
	if (act == "store_list"){
		$('#search ul.tab li span').eq(0).html('平台');
	...
	$('#search ul.tab li').eq(1).click(function(){
		$(this).hide();
		if($(this).find('span').html() == '平台') {
			$('#keyword').attr("placeholder","请输入您要搜索的平台关键字");
			$('#search ul.tab li span').eq(0).html('平台');
			$('#search ul.tab li span').eq(1).html('商品');
			$('#search_act').attr("value",'store_list');
		} else {
			...
			$('#search ul.tab li span').eq(1).html('平台');
            ...
            $.getJSON(url, {'fid':fav_id}, function(data){
                ...
                if(jstype == 'store'){
                    $('[nc_store="'+fav_id+'"]').each(function(){
                        $(this).before('<span class="goods-favorite" title="该平台已收藏"><i class="have">&nbsp;</i></span>');
                        $(this).remove();
                    });
                }
            }
