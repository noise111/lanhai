...
class PosterModel extends PluginModel
{
	...
	public function createCommissionPoster2($openid, $goodsid = 0, $type = 0)
    {
        global $_W;
        if( !$type ) 
        {
        ...
        return $this->createPoster2($poster, $member, $qr, false);
    }
	...
	public function createPoster2($poster, $member, $qr, $upload = true)
    {
        global $_W;
        $path = IA_ROOT . "/addons/ewei_shopv2/data/poster/" . $_W["uniacid"] . "/";
        if( !is_dir($path) ) 
