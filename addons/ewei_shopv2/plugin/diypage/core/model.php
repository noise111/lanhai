...
class DiypageModel extends PluginModel 
{
	...
	public function detailPage($pageid = 0) 
	{
		...
		if( !empty($detail_navbar) ) 
		{
			...
			$btnshop = array( "iconclass" => "icon-shop", "icontext" => "平台", "type" => "shop" );
				...
				else 
				{
					...
					else 
					{
						...
						else 
						{
							...
							{
								$navbar[] = array( "iconclass" => (empty($detail_navbar["params"]["shopiconclass"]) ? "icon-shop" : $detail_navbar["params"]["shopiconclass"]), "icontext" => (empty($detail_navbar["params"]["shoptext"]) ? "平台" : $detail_navbar["params"]["shoptext"]), "linkurl" => $detail_navbar["params"]["shoplink"] );
							}
						}
					}
				}
		...
		return array( "background" => $background, "followbar" => $followbar, "diygotop" => $page["data"]["page"]["diygotop"], "merch" => $page["merch"], "tab" => $detail_tab, "navbar" => $detail_navbar, "diynavbar" => $navbar, "comment" => $detail_comment, "seckill" => $detail_seckill, "diylayer" => $page["data"]["page"]["diylayer"], "items" => $pageitems, "diyadv" => $page["data"]["page"]["diyadv"], "danmu" => $page["data"]["page"]["danmu"] );
	}
