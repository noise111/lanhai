	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!(empty($item))) {
				...
				plog('goods.edit', '编辑商品 ID: ' . $id . '<br>' . ((!(empty($data['nocommission'])) ? '是否参与事业合伙人 -- 否' : '是否参与事业合伙人 -- 是')));
			}else {
				...
				$id = pdo_insertid();
				$result['id'] = $id;
				plog('goods.add', '添加商品 ID: ' . $id . '<br>' . ((!(empty($data['nocommission'])) ? '是否参与事业合伙人 -- 否' : '是否参与事业合伙人 -- 是')));
			}
