...
class Category_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('merch.category.edit', '修改特约零售商分类 ID: ' . $id);
			}
			else {
				$data['createtime'] = time();
				pdo_insert('ewei_shop_merch_category', $data);
				$id = pdo_insertid();
				plog('merch.category.add', '添加特约零售商分类 ID: ' . $id);
			}
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			...
			plog('merch.category.delete', '删除特约零售商分类 ID: ' . $item['id'] . ' 标题: ' . $item['catename'] . ' ');
		}
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			...
			plog('merch.category.edit', '修改特约零售商分类状态<br/>ID: ' . $item['id'] . '<br/>分类名称: ' . $item['catename'] . '<br/>状态: ' . $_GPC['status'] == 1 ? '显示' : '隐藏');
		}
	...
	protected function post_swipe()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('merch.category.swipe.edit', '添加特约零售商分类幻灯 ID: ' . $id);
			}
			else {
				$data['createtime'] = time();
				pdo_insert('ewei_shop_merch_category_swipe', $data);
				$id = pdo_insertid();
				plog('merch.category.swipe.add', '添加特约零售商分类幻灯 ID: ' . $id);
			}
	...
	public function delete_swipe()
	{
		...
		foreach ($items as $item) {
			...
			plog('merch.category.delete', '删除特约零售商幻灯 ID: ' . $item['id'] . ' 标题: ' . $item['catename'] . ' ');
		}
	...
	public function status_swipe()
	{
		...
		foreach ($items as $item) {
			...
			plog('merch.category.edit', '修改特约零售商幻灯状态<br/>ID: ' . $item['id'] . '<br/>分类名称: ' . $item['catename'] . '<br/>状态: ' . $_GPC['status'] == 1 ? '显示' : '隐藏');
		}
