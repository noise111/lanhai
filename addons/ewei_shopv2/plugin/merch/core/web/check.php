...
class Check_EweiShopV2Page extends PluginWebPage 
{
	...
	public function detail() 
	{
		...
		if( $_GPC["export"] == "1" ) 
		{
			...
			$columns[] = array( "title" => "特约零售商满减金额", "field" => "merchdeductenough", "width" => 24 );
			$columns[] = array( "title" => "特约零售商优惠券金额", "field" => "merchcouponprice", "width" => 24 );
	...
	public function merchpay() 
	{
		...
		if( empty($handpay) && empty($item["applytype"]) ) 
		{
			...
			$result = m("finance")->pay($merch_user["payopenid"], 1, $payprice, $item["applyno"], "特约零售商提现申请打款");
			if( is_error($result) ) 
			{
