...
class User_EweiShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        if( $_GPC["export"] == "1" ) 
        {
            ...
            plog("merch.user.export", "导出特约零售商数据");
            foreach( $list as &$row ) 
            {
            ...
            unset($row);
            m("excel")->export($list, array( "title" => "特约零售商数据-" . date("Y-m-d-H-i", time()), "columns" => array( array( "title" => "ID", "field" => "id", "width" => 12 ), array( "title" => "特约零售商名", "field" => "merchname", "width" => 24 ), array( "title" => "主营项目", "field" => "salecate", "width" => 12 ), array( "title" => "联系人", "field" => "realname", "width" => 12 ), array( "title" => "手机号", "field" => "moible", "width" => 12 ), array( "title" => "子帐号数", "field" => "accounttotal", "width" => 12 ), array( "title" => "可提现金额", "field" => "status0", "width" => 12 ), array( "title" => "已结算金额", "field" => "status3", "width" => 12 ), array( "title" => "到期时间", "field" => "accounttime", "width" => 12 ), array( "title" => "申请时间", "field" => "applytime", "width" => 12 ), array( "title" => "审核时间", "field" => "checktime", "width" => 12 ), array( "title" => "状态", "field" => "createtime", "width" => 12 ) ) ));
        }
        
        foreach($list as &$row){
            //获取运营中心数据
            if($row['agentid']){
                $agent = m('member')->getMember($row['agentid']);
                $row['agent_nickname'] = $agent['nickname'];
                $row['agent_avatar'] = $agent['avatar'];
            }
        }
    ...
    protected function post()
    {
        ...
        if( empty($id) ) 
        {
            ...
            if( $max_flag == 1 ) 
            {
                $this->message("已经达到最大特约零售商数量,不能再添加特约零售商", webUrl("merch/user"), "error");
            }
...
//            }
            ...
            {
                show_json(0, "请选择特约零售商组!");
            }

            if( empty($_GPC["cateid"]) ) 
            {
                show_json(0, "请选择特约零售商分类!");
            }
            ...
            if( empty($item) ) 
            {
                ...
                plog("merch.user.add", "添加特约零售商 ID: " . $data["id"] . " 特约零售商名: " . $data["merchname"] . "<br/>帐号: " . $data["username"] . "<br/>子帐号数: " . $data["accounttotal"] . "<br/>到期时间: " . date("Y-m-d", $data["accounttime"]));
            }
            else
            {
                if(!empty($_GPC['to_merch'])){
                    if($status == 2){
                        pdo_update("ewei_shop_saler", array("merchid" => $_GPC['to_merch']), array("merchid" => $item['id']));
                        pdo_update("ewei_shop_merch_store", array("merchid" => $_GPC['to_merch']), array("merchid" => $item['id']));
                    } else {
                        show_json(0, "只有当零售商的状态处于暂停中时才可以转移数据");
                    }
                }
                ...
                if($status == 1){
                    m('notice')->sendCommissionSuccessMessage($id);
                }
                plog("merch.user.edit", "编辑特约零售商 ID: " . $data["id"] . " 特约零售商名: " . $item["merchname"] . " -> " . $data["merchname"] . "<br/>帐号: " . $item["username"] . " -> " . $data["username"] . "<br/>子帐号数: " . $item["accounttotal"] . " -> " . $data["accounttotal"] . "<br/>到期时间: " . date("Y-m-d", $item["accounttime"]) . " -> " . date("Y-m-d", $data["accounttime"]));
            }
        ...
        //获取可用的零售商
        $avalible_merch = pdo_getall("ewei_shop_merch_user", array("status" => 1, "accounttime >" => time(), "uniacid" => $_W['uniacid']));
        include($this->template());
    }
    ...
    public function status()
    {
        ...
        foreach( $items as $item ) 
        {
            ...
            plog("merch.group.edit", ("修改特约零售商分组账户状态<br/>ID: " . $item["id"] . "<br/>特约零售商名称: " . $item["merchname"] . "<br/>状态: " . $_GPC["status"] == 1 ? "启用" : "禁用"));
        }
        show_json(1);
    }
    ...
    public function delete()
    {
        ...
        foreach( $items as $item ) 
        {
            ...
            plog("merch.user.delete", "删除`特约零售商 <br/>特约零售商:  ID: " . $item["id"] . " / 名称:   " . $item["merchname"]);
            //删除对应店铺
            pdo_delete("ewei_shop_store", array("merchid" => $item["id"], "uniacid" => $uniacid));
            //删除对应店员
            pdo_delete("ewei_shop_saler", array("merchid" => $item["id"], "uniacid" => $uniacid));
            //删除对应的店员角色
            pdo_delete("ewei_shop_saler_role", array("merchid" => $item["id"], "uniacid" => $uniacid));
        }
        show_json(1);
    }
