...
class Reg_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		if ($_GPC['export'] == '1') {
			...
			plog('merch.user.export', '导出特约零售商申请数据');
	...
	'title'   => '特约零售商数据-' . date('Y-m-d-H-i', time()),
	'columns' => array(
		array('title' => 'ID', 'field' => 'id', 'width' => 12),
		array('title' => '特约零售商名', 'field' => 'merchname', 'width' => 24),
	...
	public function detail()
	{
		...
		if (empty($item)) {
			if ($_W['ispost']) {
				show_json(0, '未找到特约零售商入驻申请!');
			}

			$this->message('未找到特约零售商入驻申请!', webUrl('merch/reg', array('status' => 0)), 'error');
		}
			...
			if ($status == 1) {
				...
				if (empty($usercount)) {
					...
					if (ca('merch.user.edit')) {
						show_json(1, array('message' => '允许入驻成功，请编辑特约零售商账户资料!', 'url' => webUrl('merch/user/edit', array('id' => $userid))));
					}
					else {
						show_json(1);
					}
					...
					if (ca('merch.user.edit')) {
						show_json(1, array('message' => '允许入驻成功，请编辑特约零售商账户资料!', 'url' => webUrl('merch/user/edit', array('id' => $usercount['id']))));
					}
					else {
						show_json(1);
					}
	...
	public function delete()
	{
		...
		foreach ($regs as $reg) {
			...
			plog('merch.reg.delete', '删除入驻申请 <br/> 特约零售商名称:  ' . $reg['merchname']);
		}
