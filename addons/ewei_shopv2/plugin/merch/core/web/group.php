...
class Group_EweiShopV2Page extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				plog('merch.group.edit', '修改特约零售商分组 ID: ' . $id);
			}
			else {
				$data['createtime'] = time();
				pdo_insert('ewei_shop_merch_group', $data);
				$id = pdo_insertid();
				plog('merch.group.add', '添加特约零售商分组 ID: ' . $id);
			}
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			...
			plog('merch.group.delete', '删除特约零售商分组 ID: ' . $item['id'] . ' 标题: ' . $item['groupname'] . ' ');
		}
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			...
			plog('merch.group.edit', '修改特约零售商分组状态<br/>ID: ' . $item['id'] . '<br/>分组名称: ' . $item['groupname'] . '<br/>状态: ' . $_GPC['status'] == 1 ? '显示' : '隐藏');
		}
	...
	public function setdefault()
	{
		...
		if (empty($group)) {
			show_json(0, '抱歉，特约零售商分组不存在或是已经被删除！');
		}

		pdo_update('ewei_shop_merch_group', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1));
		pdo_update('ewei_shop_merch_group', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $group['id']));
		plog('merch.group.setdefault', '设置默认特约零售商分组 ID: ' . $id . ' 分组名称: ' . $group['groupname']);
		show_json(1);
	}
