...
class Salerrole_EweiShopV2Page extends MerchWebPage
{
    ...
    public function post(){
        ...
        if($_W['ispost']){
            ...
            $sql = "SELECT * FROM " . tablename('ewei_shop_saler_role') . " WHERE uniacid = :uniacid AND id = :id AND merchid = :merchid";
            $parmas = array(':uniacid' => $_W['uniacid'], ':id' => $id, ':merchid' => $merchid);
    ...
    public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,rolename FROM ' . tablename('ewei_shop_saler_role') . ' WHERE id in( ' . $id . ' ) AND uniacid = ' . $_W['uniacid'] . ' AND merchid = ' . $merchid);
