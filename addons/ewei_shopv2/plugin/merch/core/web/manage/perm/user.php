...
class User_EweiShopV2Page extends MerchWebPage
{
	public function main()
	{
		...
		$psize = 20;
		$status = $_GPC['status'];
		$condition = ' and u.uniacid = :uniacid and u.merchid = :merchid ';
	...
	protected function post()
	{
		...
		if ($id) {
			$item = pdo_fetch('select u.*,r.rolename,r.merchid from ' . tablename('ewei_shop_merch_account') . ' u ' . ' left join ' . tablename('ewei_shop_merch_perm_role') . ' r on u.roleid = r.id AND r.uniacid=:uniacid AND r.deleted=0 ' . ' where u.id=:id AND u.uniacid=:uniacid AND u.merchid=:merchid ', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		}
		...
		if ($_W['ispost']) {
			$data = array(
                ...
                'roleid' => trim($_GPC['roleid']),
                'status' => trim($_GPC['status']),
                'isfounder' => 0, 
                'uniacid' => $_W['uniacid'],
                'merchid' => $_W['merchid']
            );
            isset($_GPC['openid']) ? $data['openid'] = trim($_GPC['openid']) : false;
