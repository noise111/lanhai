...
class MerchModel extends PluginModel
{
    ...
    public static $allMiniPerms = array();
    public static $formatMiniPerms = array();
    ...
    public function allPerms()
    {
        if( empty($allPerms) ) 
        {
            ...
            $perms = array( "order" => $this->perm_order(), "statistics" => $this->perm_statistics(),  "perm" => $this->perm_perm(), "apply" => $this->perm_apply(),  "exhelper" => $this->perm_exhelper());
            self::$allPerms = $perms;
        }
    ...
    public function miniPerms(){
        if( empty($allMiniPerms) ) 
        {
            $perms = array( 
                "mini" => $this->perm_mini(), 
            );
            self::$allMiniPerms = $perms;
        }

        return self::$allMiniPerms;
    }
    ...
    public function formatMiniPerms(){
        if( empty($formatMiniPerms) ) 
        {
            $perms = $this->miniPerms();
            $array = array(  );
            foreach( $perms as $key => $value ) 
            {
                if( is_array($value) ) 
            ...
            self::$formatMiniPerms = $array;
        }

        return self::$formatMiniPerms;
    }
    ...
    public function sendMessage($sendData, $message_type)
    {
        ...
        if( $message_type == "merch_apply" && empty($usernotice["merch_apply"]) ) 
        {
            if( $tm["is_advanced"] ) 
            {
                ...
                $tm["msguser"] = 0;
                $data = array( array( "name" => "特约零售商名称", "value" => $sendData["merchname"] ), array( "name" => "主营项目", "value" => $sendData["salecate"] ), array( "name" => "联系人", "value" => $sendData["realname"] ), array( "name" => "手机号", "value" => $sendData["mobile"] ), array( "name" => "申请时间", "value" => date("Y-m-d H:i:s", $sendData["applytime"]) ) );
                $tag = "merch_apply";
                $text = "[特约零售商名称]在[申请时间]提交了入驻申请，请到后台查看~";
                $time = date("Y-m-d H:i:s", time());
                $message = array( "first" => array( "value" => $sendData["merchname"] . "入驻申请", "color" => "#ff0000" ), "keyword2" => array( "title" => "业务状态", "value" => (!empty($tm["merch_applytitle"]) ? $tm["merch_applytitle"] : "特约零售商入驻申请"), "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => (!empty($tm["merch_apply"]) ? $tm["merch_apply"] : "[特约零售商名称]在[申请时间]提交了入驻申请。"), "color" => "#000000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "remark" => array( "value" => "\n请到后台查看~", "color" => "#000000" ) );
                if( !empty($openid) ) 
                {
                    foreach( $openid as $v ) 
                    {
            ...
            {
                $tm["msguser"] = 0;
                $data = array( "[特约零售商名称]" => $sendData["merchname"], "[主营项目]" => $sendData["salecate"], "[联系人]" => $sendData["realname"], "[手机号]" => $sendData["mobile"], "[申请时间]" => date("Y-m-d H:i:s", $sendData["applytime"]) );
                $message = array( "keyword1" => (!empty($tm["merch_applytitle"]) ? $tm["merch_applytitle"] : "特约零售商入驻申请"), "keyword2" => (!empty($tm["merch_apply"]) ? $tm["merch_apply"] : "[特约零售商名称]在[申请时间]提交了入驻申请，请到后台查看~") );
        ...
        if( $message_type == "merch_apply_money" && empty($usernotice["merch_apply_money"]) ) 
        {
            if( $tm["is_advanced"] ) 
            {
                ...
                $tm["msguser"] = 1;
                $data = array( array( "name" => "特约零售商名称", "value" => $sendData["merchname"] ), array( "name" => "金额", "value" => $sendData["salecate"] ), array( "name" => "联系人", "value" => $sendData["realname"] ), array( "name" => "手机号", "value" => $sendData["mobile"] ), array( "name" => "申请时间", "value" => date("Y-m-d H:i:s", $sendData["applytime"]) ) );
                $tag = "merch_applymoney";
                $text = "[特约零售商名称]在[申请时间]提交了提现申请,提现金额" . $sendData["money"] . ".[联系人] [手机号].";
                $time = date("Y-m-d H:i:s", time());
                $message = array( "first" => array( "value" => $sendData["merchname"] . "入驻申请", "color" => "#ff0000" ), "keyword2" => array( "title" => "业务状态", "value" => (!empty($tm["merch_applymoneytitle"]) ? $tm["merch_applymoneytitle"] : "特约零售商提现申请"), "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => (!empty($tm["merch_apply"]) ? $tm["merch_apply"] : "[特约零售商名称]在[申请时间]提交了提现申请,提现金额" . $sendData["money"] . ".[联系人] [手机号]."), "color" => "#000000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "remark" => array( "value" => "\n请到后台查看~", "color" => "#000000" ) );
                if( !empty($openid) ) 
                {
                    foreach( $openid as $v ) 
                    {
            ...
            {
                $tm["msguser"] = 1;
                $data = array( "[特约零售商名称]" => $sendData["merchname"], "[金额]" => $sendData["money"], "[联系人]" => $sendData["realname"], "[手机号]" => $sendData["mobile"], "[申请时间]" => date("Y-m-d H:i:s", $sendData["applytime"]) );
                $message = array( "keyword1" => (!empty($tm["merch_applymoneytitle"]) ? $tm["merch_applymoneytitle"] : "特约零售商提现申请"), "keyword2" => (!empty($tm["merch_applymoney"]) ? $tm["merch_applymoney"] : "[特约零售商名称]在[申请时间]提交了提现申请,提现金额" . $sendData["money"] . ".[联系人] [手机号].请到后台查看~") );
    ...
    {
        global $_W;
        $condition = " and b.uniacid=:uniacid and b.id=:id";
    ...
    public function checkMaxMerchUser($type = 0)
    {
        ...
        if( 0 < $max_merch && $max_merch <= $totals ) 
        {
            ...
            {
                show_json(0, "已经达到最大特约零售商数量,不能再添加特约零售商.");
            }
