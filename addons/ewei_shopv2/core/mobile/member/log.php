...
class Log_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function get_list() 
	{
		...
		$uid = $uidinfo["uid"];
		$credit_condition = " and r.uniacid=" . $_W["uniacid"] . " and r.credittype='credit2' and r.uid = " . $uid . " and r.num > 0 order by r.createtime desc ";
		$r = pdo_fetchall("select m.uid,m.mobile,m.nickname,r.remark title,r.num money,r.createtime from " . tablename("ewei_shop_member_credit_record") . "r left join " . tablename("ewei_shop_member") . " m on m.uid = r.uid where 1 " . $credit_condition);
		foreach( $r as &$item ) 
		{
