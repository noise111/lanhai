...
class Verify_EweiShopV2ComModel extends ComModel 
{
	...
	public function allow($orderid, $times = 0, $verifycode = "", $openid = "") 
	{
		...
		else 
		{
			if( $merch_plugin ) 
			{
				$saler = pdo_fetch("select * from " . tablename("ewei_shop_saler") . " where openid=:openid and uniacid=:uniacid and status=1 and merchid=:merchid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid, ":merchid" => $merchid ));
			}
		}
		if( empty($saler) ) 
		{
