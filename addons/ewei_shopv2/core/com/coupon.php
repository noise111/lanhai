...
class Coupon_EweiShopV2ComModel extends ComModel 
{
	...
	public function getAvailableCoupons($type, $money = 0, $merch_array, $goods_array = array( )) 
	{
		...
		$sql = "select d.id,d.couponid, d.sender, d.openid, d.gettime, d.gettype, c.commissiontype, c.backcommission,
                c.timelimit,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,
                c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.limitgoodcatetype,
                c.limitgoodtype,c.limitgoodcateids,c.limitgoodids  from " .
                tablename("ewei_shop_coupon_data") . " d";
		$sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
		$sql .= " where d.uniacid=:uniacid and c.merchid=0 and d.merchid=0 and (c.coupontype=" . $type . " OR c.coupontype = ". COUPON_SOCIAL .") and d.used=0 ";
		if( $type == 1 ) 
		{
        ...
        $order = " order by d.gettime asc, id asc ";
        $list1 = pdo_fetchall($sql . $condition . $order, $param);
        empty($list1) ? $list1 = array() : false;
        //分享得到的券
        $condition = " and (d.openid = :openid and d.sender <> :openid )";
        $list2 = pdo_fetchall($sql . $condition . $order, $param);
        empty($list2) ? $list2 = array() : false;
		...
		if( !empty($merch_array) ) 
		{
			foreach( $merch_array as $key => $value ) 
			{
				...
				if( 0 < $merchid ) 
				{
					...
					$sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
					$sql .= " where d.openid=:openid and d.uniacid=:uniacid and c.merchid=:merchid and d.merchid=:merchid and (c.coupontype=" . $type . " OR c.coupontype = ". COUPON_SOCIAL .") and d.used=0 ";
	...
	public function consumeCouponCount($openid, $money = 0, $merch_array, $goods_array) 
	{
		...
		$sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
		$sql .= " where (d.openid=:openid OR d.sender=:openid) and d.uniacid=:uniacid and c.merchid=0 and d.merchid=0 and 
                 (c.coupontype=0 OR c.coupontype = ". COUPON_SOCIAL .") and d.used=0 ";
		$sql .= " and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timestart<=" . $time . " && c.timeend>=" . $time . ")) order by d.gettime desc";
		...
		if( !empty($merch_array) ) 
		{
			foreach( $merch_array as $key => $value ) 
			{
				...
				if( 0 < $merchid ) 
				{
					...
					$sql = "select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids  from " . tablename("ewei_shop_coupon_data") . " d";
					$sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
					$sql .= " where (d.openid=:openid OR d.sender=:openid) and d.uniacid=:uniacid and c.merchid=:merchid and d.merchid=:merchid and c.coupontype=0  and d.used=0 ";
					$sql .= " and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timestart<=" . $time . " && c.timeend>=" . $time . "))";
					$merch_list = pdo_fetchall($sql, $param);
					if( !empty($merch_list) ) 
					{
	...
	public function useConsumeCoupon($orderid = 0) 
	{
		...
		$order = pdo_fetch("select ordersn,createtime,couponid, multicoupon from " . tablename("ewei_shop_order") . " where id=:id and status>=0 and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
		if( empty($order) ) 
		{
			return NULL;
		}
        //一张订单使用了多张优惠券
        if($order['multicoupon'] == 1){
            $params = array(
                ':uniacid'       => $_W['uniacid'],
                ':orderid'       => $orderid
            );
            $sql = "SELECT couponid FROM " . tablename('ewei_shop_order_goods') . " WHERE uniacid = :uniacid AND orderid = :orderid AND couponid > 0 ";
            //coupon_data表的id
            $coupons = pdo_fetchall($sql, $params, 'couponid');
            ...
            if($coupon_ids && !empty($coupon_ids)){
                ...
                $sql = "UPDATE " . tablename('ewei_shop_coupon_data') . " SET used = 1, usetime = {$order['createtime']}, ordersn = '{$order['ordersn']}' $openid WHERE id IN(" . implode(',', $coupon_ids) . ")";
                pdo_query($sql);
            } else {
                return NULL;
            }                      
	...
	public function returnConsumeCoupon($order) 
	{
        ...
        pdo_update("ewei_shop_coupon_data", array( "used" => 0, "usetime" => 0, "ordersn" => "" ), array( "ordersn" => $order["ordersn"] ));
	...
	{
		global $_W;
		$data = pdo_fetch("select id, openid, couponid, used, back, backtime, ordersn, sender from " . tablename("ewei_shop_coupon_data") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $dataid, ":uniacid" => $_W["uniacid"] ));
        if( empty($data) ) 
		{
			return false;
		}
	...
	public function posterbylist($list, $openid, $gettype) 
	{
		...
		foreach( $list as $taskdata ) 
		{
			...
			$num += $couponnum;
            $coupon = $this->getCoupon($taskdata['couponid']);
			for( $i = 1; $i <= $couponnum; $i++ ) 
			{
				$couponlog = array( 
                    "uniacid" => $_W["uniacid"], 
                ...
                if($coupon['coupontype'] == COUPON_SOCIAL){
                    $data['sender'] = $openid;
                    $data['openid'] = $openid;
                    $data['sharebatch'] = md5($openid . $now);
                }
     ...
     * 获取店员社交券
     */
    public function getSalerSocialCoupon($openid){
        global $_W;
        $field = "d.*, c.couponname, c.timedays, c.deduct";
        $sql = "SELECT $field FROM " . tablename('ewei_shop_coupon_data') ." d
                INNER JOIN " . tablename('ewei_shop_coupon') . " c ON c.id = d.couponid AND c.uniacid = :uniacid AND c.coupontype = " . COUPON_SOCIAL .
               " WHERE d.sender = :openid AND d.uniacid = :uniacid AND d.used = 0
                ORDER BY d.id ASC";
        $list = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
        return $list;
    }
    
    public function setCouponRelation($orderid = 0){
        if($orderid){
            $sql = " SELECT id, uniacid, openid, saleropenid, couponid FROM " . tablename('ewei_shop_order') . " WHERE id = $orderid";
            $order = pdo_fetch($sql);
            if(!$order){
                return NULL;
            }
            $sql = " SELECT id, saleropenid, customeropenid, parentopenid FROM " . tablename('ewei_shop_coupon_relation') . " WHERE customeropenid = '{$order['openid']}' AND uniacid = {$order['uniacid']}";
            $row = pdo_fetch($sql);
            //此用户之前已经消费过
            if($row){
                ...
                if(!empty($order['saleropenid'])){  
                    ...
                    pdo_update('ewei_shop_coupon_relation', $update, array('id' => $row['id']));
                    m('notice')->customerBindSalerSuccess($row['id']);
                    return NULL;
                }                
            }
            ...
            $data['customeropenid'] = $order['openid'];
            $data['createtime']     = time();
            //店员引导的用户消费
            if(!empty($order['saleropenid'])){               
                $data['saleropenid']    = $order['saleropenid'];   
            //可能是用户引导其他用户的消费，读取优惠券信息
            } else {
                $sql = " SELECT couponid FROM " . tablename('ewei_shop_order_goods') . " WHERE orderid = $orderid AND openid = '{$order['openid']}' AND uniacid = {$order['uniacid']}";
                $coupon_data = pdo_fetchall($sql, array(), 'couponid');
                $coupon_ids = array_keys($coupon_data);
                $order['couponid'] ? $coupon_ids[] = $order['couponid'] : false;
                if($coupon_ids && !empty($coupon_ids)){
                    $sql = "SELECT sender FROM " . tablename('ewei_shop_coupon_data') . " WHERE sender != '' AND id IN(" . implode(',', $coupon_ids) .") ORDER BY gettime asc";
                    $coupon = pdo_fetchall($sql);
                    if($coupon){
                        $sender = $coupon[0]['sender'];
						//检测sender是否是导购
						$saler_info = m('store')->getSalerInfo(array('openid' => $sender));
						if($saler_info){
							$data['saleropenid'] = $sender;
						} else {
							//获取sender的店员
							$sql = "SELECT * FROM " . tablename('ewei_shop_coupon_relation') . " WHERE customeropenid = '{$sender}'";
							$customer_saler = pdo_fetch($sql);
							//如果有记录则是店员
							if($customer_saler){
								$data['saleropenid'] = $customer_saler['saleropenid'];
							} 
							$data['parentopenid'] = $sender;
						}                       
                    }
            ...
            if($row){
                pdo_update("ewei_shop_coupon_relation", $data, array('id' => $row['id']));
                m('notice')->customerBindSalerSuccess($row['id']);
            } else {
                pdo_insert('ewei_shop_coupon_relation', $data); 
                $insert_id = pdo_insertid();
                m('notice')->customerBindSalerSuccess($insert_id);
            }
        }
    }
    ...
    /**
     * 检测是否送社交券
     * @param string $openid 要送券的目标id
     * @param string $scene 送券场景
     * @param array $check_data 送券条件数组
     * autotarget   1：门店店员 2：用户 3：其他
     */
    public function checkSocialPoint($openid, $scene = '', $check_data = array()){
        global $_W;
        $condition = " uniacid = " . $_W['uniacid'] . " AND autonumber > 0 AND coupontype = " . COUPON_SOCIAL;
        $send = false;
        $insert_data = array();
        $gettype = COUPON_BACKEND;
        switch ($scene){
            //新店员审核通过
            case 'newpass':
                if($check_data['pass'] == 1 && empty($check_data['passtime'])){
                    $send = true;
                    $condition .= " AND isnewpass = 1 AND autotarget = 1 ";
                    $gettype = COUPON_NEWGET;
                }
                break;
            case 'finishorder':
                if($check_data['order_status'] == 3){
                    $send = true;
                    $condition .= " AND finishorder = 1 ";
                }
                $gettype = COUPON_BUY;
                break;
        }
        if($send){
            $now = time();
            $sql = " SELECT id, autotarget, autonumber, autogoodsid, autotype FROM " . tablename('ewei_shop_coupon') . " WHERE " . $condition;
            $coupons = pdo_fetchall($sql);
            if($coupons){
                foreach ($coupons as $coupon){
                    $number = 0;
                    //购买指定商品才送券
                    if(!empty($coupon['autogoodsid'])){
                        $autogoodsids = explode(',', $coupon['autogoodsid']); 
                        if($check_data['orderid']){
                            $sql = "SELECT goodsid FROM " . tablename("ewei_shop_order_goods") . " WHERE orderid = {$check_data['orderid']}";
                            $goodsids = array_keys(pdo_fetchall($sql, array(), 'goodsid'));
                            if(empty(array_intersect($autogoodsids, $goodsids))){
                                continue;
                            }
                        } else {
                            continue;
                    ...
                    switch($scene){
                        case 'newpass':
                            ...
                            if($coupon['autotype'] == 1 && $saler_info['shoppingguide'] == 0){
                                continue;
                            } else if($coupon['autotype'] == 2 && $saler_info['storemanager'] == 0){
                                continue;
                            }
                            break;
                        case 'finishorder':
                            if($coupon['autotarget'] == 1){
                                $saler_info = m('store')->getSalerInfo(array('openid' => $openid));
                                if($coupon['autotype'] == 1 && $saler_info['shoppingguide'] == 0){
                                    continue;
                                } else if($coupon['autotype'] == 2 && $saler_info['storemanager'] == 0){
                            ...
                            if($coupon['autotarget'] == 2 && $coupon['autotype'] == 2 && $check_data['orderid']){
                                $sql = "SELECT SUM(total) FROM " . tablename("ewei_shop_order_goods") . " WHERE orderid = {$check_data['orderid']}";
                                $count = pdo_fetchcolumn($sql);
                                $number = empty($count) ? 0 : intval($count) * intval($coupon['autonumber']);
                            }
                            break;
                    }
                                       
                    $number = empty($number) ? $coupon['autonumber'] : $number;
                    $data = array(
                        'uniacid'       => $_W['uniacid'],
                        'sender'        => $openid,
                        'openid'        => $openid,
                        ...
                        'gettype'       => $gettype,
                        'gettime'       => $now,
                        'sharebatch'    => microtime(true) * 10000
                    );
                    $data = array_merge($data, $insert_data);
                    for($i = 0; $i < $number; $i++){
                        pdo_insert('ewei_shop_coupon_data', $data);
                    }
                    m("notice")->sendCouponMessage($openid);
                }
            }
        }
    }
    
    public function setCouponBenefit($orderid){
        global $_W;
        $sql = "SELECT * FROM " . tablename("ewei_shop_order") . " WHERE id = $orderid AND uniacid = {$_W['uniacid']}";
        $order = pdo_fetch($sql);
        //普通购买流程使用了优惠券
        if($order && !empty($order['couponid'])){
            $order_coupon = $this->getCouponByDataID($order['couponid']);
            if(!empty($order_coupon['sender'])){
                $log_data = array(
                    'customeropenid'    => $order['openid'],
                    'couponid'          => $order['couponid'],
                    'orderid'           => $orderid
                );
                $value = $order_coupon['backcommission'] == '-1' ? $order_coupon['deduct'] : $order_coupon['backcommission'];
                m('member')->setBenefit($order_coupon['sender'], $order_coupon['commissiontype'], $value, $log_data);
                m('notice')->sendCouponBenefitMessage($orderid);
            }
        }
        //导购模式购物流程使用了优惠券
        $sql = "SELECT c.commissiontype, c.backcommission, c.deduct, cd.sender, cd.openid, cd.id
                FROM " . tablename('ewei_shop_order_goods') . " og " .
               "INNER JOIN " . tablename('ewei_shop_coupon_data') . " cd ON cd.id = og.couponid " .
               "INNER JOIN " . tablename('ewei_shop_coupon') . " c ON c.id = cd.couponid AND c.commissiontype >= 0
        ...
        if($data){                    
            ...
            foreach($data as $item){
                $openid = $item['sender'];
                if($item['backcommission'] == '-1'){
                    $item['backcommission'] = $item['deduct'];
                }
                if($openid){
                    $log_data = array(
                        'customeropenid'    => $order['openid'],
                        ...
                        'number'            => $item['backcommission'],
                        'createtime'        => $now
                    );
//                    $remark = "社交券返：" . $item['backcommission'];
...
//                    $log_data['remark'] = $remark;
                    m('member')->setBenefit($openid, $item['commissiontype'], $item['backcommission']);
                    pdo_insert('ewei_shop_coupon_benefit_log', $log_data);    
                }               
            }
            m('notice')->sendCouponBenefitMessage($orderid);
        }
    ...
    public function getAvailableCount($openid){
        global $_W;
        $time = time();
        $sql = "select count(*) from " . tablename("ewei_shop_coupon_data") . " d";
        $sql .= " left join " . tablename("ewei_shop_coupon") . " c on d.couponid = c.id";
        $sql .= " where (d.openid=:openid OR d.sender =:openid) and d.uniacid=:uniacid and  d.used=0 ";
        ...
        $result = pdo_fetchcolumn($sql, array(":openid" => $openid, ":uniacid" => $_W['uniacid']));
        return $result;
    }
}
?>