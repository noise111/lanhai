...
class Common_EweiShopV2Model 
{
    ...
    public function getAreaCode($province, $city = '', $area =''){
        $areas = $this->getAreas();
        $province_list = $areas['province'];
        $code = array(
            'province'  => '0',
            'city'      => '0',
            'area'      => '0'
        ...
        foreach($province_list as $item){
            if($item['@attributes']['name'] == $province){
                $code['province'] = $item['@attributes']['code'];
                if($city){
                    foreach($item['city'] as $i_city){
                        if($i_city['@attributes']['name'] == $city){
                            $code['city'] = $i_city['@attributes']['code'];
                        }
                        if($area){
                            foreach($i_city['county'] as $i_area){
                                if($i_area['@attributes']['name'] == $area){
                                    $code['area'] = $i_area['@attributes']['code'];
                                }
                            }
                        }
                    }
     ...
     * @return type
     */
    public function array_key_field($arr, $field = "id"){
        $temp = array();
