...
class Qrcode_EweiShopV2Model
{
    ...
    public function getCodeLog($qid, $no){
        global $_W;
        $search = array(
            ":uniacid"      => $_W['uniacid'],
            ":qid"          => $qid,
        ...
        $sql = "SELECT * FROM " . tablename('ewei_shop_goods_qrcode_log') . " WHERE qid = :qid AND uniacid = :uniacid AND no = :no";
        $log = pdo_fetch($sql, $search);  
        return $log;
    }
    
    public function getQrcodeUrl($qrcodeid, $qrcodeno){
        global $_W;
        $path = $_W['siteroot'] . '/addons/ewei_shopv2/data/app_qrcode/'. $_W['uniacid'] . "/$qrcodeid/" . $qrcodeno . '.png';
        return $path;
    }
}
