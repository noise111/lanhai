	...
	public function getMember($openid = "") 
	{
		...
		if( $uid == 0 ) 
		{
            ...
            if(!empty($openid)){
                ...
                if( empty($info) ) {  
                    //新增原openid搜索
                    $info = pdo_fetch("select * from " . tablename("ewei_shop_member") . " where  ori_openid=:openid and uniacid=:uniacid limit 1", array( ":uniacid" => $_W["uniacid"], ":openid" => $openid )); 
                    if(empty($info)){
                        if( strexists($openid, "sns_qq_")){
                            $openid = str_replace("sns_qq_", "", $openid);
                            $condition = " openid_qq=:openid ";
...
//		}
        ...
        //用户是否存在（有可能记录了unionid而没有记录openid）
        $member_exists = false;
        $ori_openid = $openid;
		if( empty($member) && !empty($openid) ) 
		{
            if(isset($mc['unionid']) && !empty($mc['unionid'])){
                $member_tmp = $this->getMember($mc['unionid']);
                if($member_tmp){
                    $member_exists = true;
                    $openid = $member_tmp['openid'];
                    $_W['openid'] = $openid;
                    $GLOBALS['_W']['openid'] = $openid;
                } else {
                    $member_exists = false;
                }
            }
            if(!$member_exists){
                ...
                if(isset($mc['unionid']) && !empty($mc['unionid'])){
                    $member['ori_openid'] = $ori_openid;
                    $member['openid'] = $mc['unionid'];
                    $openid = $mc['unionid'];
                }
		...
		if($member || $member_exists) {
            ...
            if(empty($member['ori_openid']) && !strexists($ori_openid, "sns_qq_") && !strexists($ori_openid, "sns_wx_") && !strexists($ori_openid, "sns_wa_")){               
                //不是sns的openid才更新
                $upgrade['ori_openid'] = $ori_openid;
            }           
            if(isset($mc['unionid']) && !empty($mc['unionid'])){
                $openid = $mc['unionid'];
                ...
                if($member['openid'] != $mc['unionid']){
                    $member['openid'] = $mc['unionid'];
                    $upgrade['openid'] = $mc['unionid'];
                }
            }  
                       
            m("unionid")->checkUnionBind($ori_openid, $mc['unionid']);
			if( !empty($upgrade) ) 
			{
    ...
    public function setBenefit($openid, $type, $value = 0, $remark = ''){    
        global $_W;
        $sql = "SELECT id, credit1, credit2, benefitbalance FROM " . tablename('ewei_shop_member') . " WHERE openid = '%s'";
        $member = pdo_fetch(sprintf($sql, $openid));
        $now = time();
        switch ($type) {
            //推广佣金
            case 0:
                $update = array(
                    'benefitbalance' => floatval($member['benefitbalance']) + floatval($value)
                );
                pdo_update('ewei_shop_member', $update, array('openid' => $openid));                               
                $ben_log = array(
                    'openid'    => $openid,
                    'num'       => $value,
                    'remark'    => '社交券返：' . $value
                );
                empty($remark) ? : $ben_log['remark'] = $remark;
                $ben_log['credittype'] = 'benefit';
                $ben_log['createtime'] = $now;
                $ben_log['uniacid'] = $_W['uniacid'];
                $ben_log['module'] = 'ewei_shopv2';
                pdo_insert('ewei_shop_member_credit_record', $ben_log);
                break;
            //积分
            case 1:
                m('member')->setCredit($openid, 'credit1', $value, '社交券返：' . $value . '积分');
                break;
            //余额
            case 2:
                m('member')->setCredit($openid, 'credit2', $value, '社交券返：' . $value . '余额');
                break;
            //微信红包
            case 3:
                break;
    ...
    public function getMemberBase($openid){
        $uid = intval($openid);
        $where = array();
        if($uid > 0){
            $where['id'] = $uid;
        ...
        return pdo_get("ewei_shop_member", $where, array('id', 'openid', 'ori_openid', 'nickname', 'realname', 'avatar', 'avatar_wechat', 'mobile'));
    }
    
    public function getMerchInfo($openid){
        $info = array(
            "is_merch" => 0,
            "list"     => array()
        );
        ...
        if(!empty($openid)){
            $users = array();
            $account = pdo_getall("ewei_shop_merch_account", array("openid" => $openid), array(), "merchid");
            if($account){
                $merchids = array_keys($account);
                $merchids_where = implode(",", $merchids);
                if(!empty($merchids_where)){
                    $users1 = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_merch_user") . 
                                          " WHERE id IN ($merchids_where) AND status = 1 AND accounttime >= $now ",
                                          array(),
                                          "id"
                            );
                    $users = !empty($users1) ? array_merge($users, $users1) : $users;
                }
                
            }
            $users2 = pdo_getall("ewei_shop_merch_user", array("openid" => $openid, "status" => 1, "accounttime >=" => $now), array(), "id");
            $users = !empty($users2) ? array_merge($users, $users2) : $users;
            $users = m("common")->array_key_field($users, "id");
            if($users){
                foreach($users as &$user){
                    if(isset($account[$user['id']])){
                        $user['merchid'] = isset($account[$user['id']]['merchid']) ? $account[$user['id']]['merchid'] : null;
                        $user['account_status'] = isset($account[$user['id']]['status']) ? $account[$user['id']]['status'] : 0;
                        $user['isfounder'] = isset($account[$user['id']]['isfounder']) ? $account[$user['id']]['isfounder'] : 0;
                        $user['roleid'] = isset($account[$user['id']]['roleid']) ? $account[$user['id']]['roleid'] : 0;
                        $user['account_id'] = $account[$user['id']]['id'];
                    } else {
                        $user['merchid'] = null;
                    ...
                    if($user['status'] == 1 && $user['accounttime'] >= $now){
                        $info['is_merch'] = 1;
                    }
                }
                $info['list'] = $users;
    ...
    public function getAgentInfo($openid){
        ...
        if(!empty($openid)){
            $members = array();
            $account = pdo_getall("ewei_shop_agent_account", array("openid" => $openid), array(), "agentid");
            if($account){
                $agent_ids = array_keys($account);
                $agent_where = implode(",", $agent_ids);
                if(!empty($agent_where)){
                    $members1 = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_member") . 
                                            " WHERE id IN($agent_where) AND isaagent = 1 AND aagentstatus = 1",
                                            array(),
                                            "id"
                              );
                    $members = !empty($members1) ? array_merge($members, $members1) : $members;
                }
            }
            $members2 = pdo_getall("ewei_shop_member", array("openid" => $openid, "isaagent" => 1, "aagentstatus" => 1), array(), "id");
            $members = !empty($members2) ? array_merge($members, $members2) : $members;
            $members = m("common")->array_key_field($members, "id");
            if($members){
                foreach($members as &$member){
                    if(isset($account[$member['id']])){
                        $member['account_id'] = isset($account[$member['id']]['id']) ? $account[$member['id']]['id'] : '';
                        $member['account_openid'] = isset($account[$member['id']]['openid']) ? $account[$member['id']]['openid'] : '';
                        $member['account_status'] = isset($account[$member['id']]['status']) ? $account[$member['id']]['status'] : 0;
                    } else {
                        $member['account_id'] = 0;
                        $member['account_openid'] = '';
