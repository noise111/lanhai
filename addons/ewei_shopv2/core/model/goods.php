...
class Goods_EweiShopV2Model 
{
	...
	public function getList($args = array( )) 
	{
		...
		if( !$random ) 
		{
			$sql = "SELECT id,title,subtitle,thumb,thumb_url" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY " . $order . " " . $orderby . " LIMIT " . ($page - 1) * $pagesize . "," . $pagesize;
			$total = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_goods") . " where  " . $condition . " ", $params);
		}
		else 
		{
			$sql = "SELECT id,title,thumb,thumb_url" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY rand() LIMIT " . $pagesize;
			$total = $pagesize;
		}
    ...
    public function getCouponsByGood($goodid, $coupon_type = array()) {
        ...
        if(empty($coupon_type)){
            ...
            $coupon_type = " and coupontype IN(" . implode(',', $coupon_type) . ")";
        }
		$sql = "select id,timelimit,coupontype,timedays,timestart,timeend,thumb,couponname,enough,backtype,deduct,discount,backmoney,backcommission,
                backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,islimitlevel,limitmemberlevels,limitagentlevels,
                limitpartnerlevels,limitaagentlevels,limitgoodcatetype,limitgoodcateids,limitgoodtype,limitgoodids,tagtitle,settitlecolor,titlecolor 
                from " . tablename("ewei_shop_coupon") . " c ";
        
		$sql .= " where uniacid=:uniacid and money=0 and credit = 0 " . $coupon_type;
		...
		$sql .= " and (total=-1 or total>0) and ( timelimit = 0 or  (timelimit=1 and timeend>" . $time . "))";
     ...
     * 计算商品用券后的数据
     */
    public function calculate_goods_coupon($goodsid, $couponid){
        global $_W;
        $uniacid = $_W['uniacid'];
        
        $sql = "SELECT marketprice, productprice, costprice, originalprice FROM " . tablename("ewei_shop_goods") . " WHERE uniacid = $uniacid AND id = $goodsid";
        $goods = pdo_fetch($sql);
        if($goods){
            $return = array(
                'deductprice'       => 0,
                'marketprice'       => $goods['marketprice'],
                'couponprice'       => $goods['marketprice']
            );
            $params = array(
                ':uniacid'      => $uniacid,
                ':id'           => $couponid
     ...
     * 根据商品id自动选择合适的优惠券
     * (优先用自己的，然后分享的来的券)
     * @param array $selected 已被选择的优惠券id数组
     */
    public function get_auto_goods_coupon($openid, $goodsid, $selected = array()){
        global $_W;
        $time = time();
        $goods_info = pdo_get("ewei_shop_goods", array("id" => $goodsid));
        $goods_cates = $goods_info['cates'];
        $goods_cates = explode(',', $goods_cates);
        if(empty($goods_info)){
            return false;
        }
        $condition = "d.openid = '$openid'";
        $condition .= " and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timestart<=" . $time . " && c.timeend>=" . $time . ")) ";
        if(!empty($selected)){
            $condition .= " AND d.id NOT IN(" . implode(',', $selected) . ") ";
        }
        //商品分类限制
        if(!empty($goods_cates)){
            $cates_condition = "";
            foreach($goods_cates as $cate){
                $cates_condition .= " FIND_IN_SET($cate, c.limitgoodcateids) OR";
            }
            $cates_condition = rtrim($cates_condition, "OR");
            if(!empty($cates_condition)){
                $cates_condition = " AND (c.limitgoodcateids = '' OR ($cates_condition)) ";
            }           
        }
        $condition .= $cates_condition;
        //商品限制
        $condition .= " AND (c.limitgoodids = '' OR FIND_IN_SET($goodsid, c.limitgoodids))";
        
        $select = "SELECT d.id FROM " . tablename("ewei_shop_coupon_data") . " d " .
                  " LEFT JOIN " . tablename("ewei_shop_coupon") . " c ON d.couponid = c.id " .
                  " WHERE d.uniacid = {$_W['uniacid']} AND d.used = 0 AND ";
        //自己的券
        $sql = $select . $condition . " AND d.sender = '$openid' LIMIT 1";
