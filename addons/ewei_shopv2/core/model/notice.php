			...
			if( $order["status"] == 1 ) 
			{
				...
				else 
				{
					if( $order["success"] == -1 ) 
					{
						...
						$msg = array( "first" => array( "value" => "您参加的拼团组团失败！", "color" => "#000000" ), "keyword1" => array( "title" => "平台", "value" => $shopname, "color" => "#000000" ), "keyword2" => array( "title" => "通知时间", "value" => date("Y-m-d H:i:s", time()), "color" => "#000000" ), "keyword3" => array( "title" => "商品", "value" => $order_goods["title"], "color" => "#000000" ), "remark" => array( "value" => $remark, "color" => "#000000" ) );
						foreach( $order as $key => $value ) 
						{
...
//			$goodsname .= $og["title"] . " \n\n";
//			$goodsnum += $og["total"];
             if(!isset($tmp_goods[$og['id']])){
                $goods_array = array(
                    'title'         => $og['title'],
                    'price'         => $og['price'],
                    'total'         => $og['total'],
                    'realprice'     => $og['realprice']
                );
                $tmp_goods[$og['id']] = $goods_array;                
            } else {
                $tmp_goods[$og['id']]['total'] += $og['total'];
            }
		}
        foreach($tmp_goods as $og){
            $goods .= "\n\n" . $og['title'] . "(";
            $goods .= " 单价: " . $og["price"] . " 数量: " . $og["total"] . " 总价: " . $og["price"] * $og["total"] . "); ";
            $goodsname .= $og["title"] . " \n\n";
            $goodsnum += $og["total"];
        }
		$couponstr = "";
		...
		$datas = array( 
            ...
            array( 
                "name" => "购买者姓名", 
                "value" => $buyerinfo_name 
            ), 
            array( 
                "name" => "购买者电话", 
                "value" => $buyerinfo_mobile 
            ), 
            array( 
                "name" => "收货地址", 
                "value" => $addressinfo 
            ...
            array( 
                "name" => "取消时间", 
                "value" => date("Y-m-d H:i", $order["canceltime"])
            ), 
            array( 
                "name" => "门店", 
                "value" => (!empty($store) ? $store["storename"] : "") 
            ), 
            array( 
                "name" => "门店地址", 
                "value" => (!empty($store) ? $store["address"] : "") 
            ), 
            array( 
                "name" => "门店联系人", 
                "value" => (!empty($store) ? $store["realname"] . "/" . $store["mobile"] : "") 
            ), 
            array( 
                "name" => "门店营业时间", 
                "value" => (!empty($store) ? (empty($store["saletime"]) ? "全天" : $store["saletime"]) : "") 
            ), 
            array( 
                "name" => "虚拟物品自动发货内容",
                "value" => $order["virtualsend_info"] 
            ), 
            array( 
                "name" => "虚拟卡密自动发货内容", 
                "value" => $order["virtual_str"] 
            ), 
            array( 
                "name" => "自提码", 
                "value" => $order["verifycode"] 
            ), 
            array( 
                "name" => "备注信息", 
                "value" => $order["remark"] 
								...
								if( empty($order["dispatchtype"]) && !$isonlyverify ) 
								{
									...
									if( empty($order["sendtype"]) ) 
									{
										...
										if( 0 < $order["merchid"] ) 
										{
											...
											{
												$datas[] = array( "name" => "特约零售商电话", "value" => $merch_user["mobile"] );
												$text .= "\n特约零售商电话：[特约零售商电话]";
												$remark_value .= "\n特约零售商电话：" . $merch_user["mobile"];
											}
											if( !empty($merch_user["address"]) ) 
											{
												$datas[] = array( "name" => "特约零售商地址", "value" => $merch_user["address"] );
												$text .= "\n特约零售商地址：[特约零售商地址]";
												$remark_value .= "\n特约零售商地址：" . $merch_user["address"];
											}
										}
										$text .= $remark;
										$remark_value .= "\n我们正加速送到您的手上，请您耐心等候";
	...
	public function sendNotice(array $params) 
	{
		...
		$templateid = $tm_temp;
        isset($params['templateid']) ? $templateid = isset($params['templateid']) :  false;
        $templatecode = isset($params['templatecode']) ? trim($params['templatecode']) : "";
		$datas = (isset($params["datas"]) ? $params["datas"] : array( ));
		$default_message = (isset($params["default"]) ? $params["default"] : array( ));
		...
		if( !empty($templateid) || !empty($templatecode) ) 
		{
            $tid_where = "id = $templateid ";
            if(!empty($templatecode)){
                $tid_where = " typecode = '$templatecode' ";
            }
			$template = pdo_fetch("select * from " . tablename("ewei_shop_member_message_template") . " where $tid_where and uniacid=:uniacid limit 1", array(":uniacid" => $_W["uniacid"] ));
            if( !empty($template) ) 
			{
     ...
     * @return type
     */
    public function sendOrderCreateMessage($orderid){
        $order = m('order')->getOrderDetail($orderid);
        if($order['status'] != 0){
            return false;
        }
        $address = $order['address_detail'] ? $order['address_detail'] : "";
        $templatecode = "saler_goodsubmit"; 
        $appurl = "pages/order/detail/index?id=" . $orderid;
        $goods_detail = "";
        $tmp_goods = array();
        foreach($order['order_goods'] as $goods){
            if(!isset($tmp_goods[$goods['goodsid']])){
                $goods_array = array(
                    'title'         => $goods['title'],
                    'price'         => $goods['price'],
                    'total'         => $goods['total']
                );
                $tmp_goods[$goods['goodsid']] = $goods_array;                
            } else {
                $tmp_goods[$goods['goodsid']]['total'] += $goods['total'];
            }
        }
        foreach($tmp_goods as $goods){
            $goods_detail .= $goods['title'] . "(单价：" . $goods['price'] . " 数量：" . $goods['total'] . " 总价：" . $goods['price'] * $goods['total'] . ")";
            $goods_detail = trim($goods_detail, ',');
        }
        $data = array(
            array("name" => "商品详情", "value" => $goods_detail),
            array("name" => "下单时间", "value" => date("Y-m-d H:i:s", $order['createtime'])),
            array("name" => "收货地址", "value" => $address),
            array("name" => "购买者姓名", "value" => $order['nickname']),
            array("name" => "支付状态", "value" => $order['pay_status']),
            array("name" => "备注信息", "value" => "订单总价：" . $order['price'])
        );
     ...
     * 优惠券分润收入提醒
     */
    public function sendCouponBenefitMessage($orderid){
        $benefit_log = pdo_getall("ewei_shop_coupon_benefit_log", array("orderid" => $orderid));
        foreach($benefit_log as $log){
            $type = "钱包";
            switch($log['type']){
                case 0:
                    $type = "佣金";
            ...
            $params = array(
                "openid"        => $log['openid'],
                "appurl"        => "pages/member/index/index",
                "templatecode"  => "coupon_benefit_message",
                "datas"         => array(
                    array("name" => "返佣金额", "value" => $log['number']),
                    array("name" => "返佣类型", "value" => $type),
                    array("name" => "返佣时间", "value" => date("Y-m-d H:i:s", $log['createtime']))
                )
            );
     ...
     */
    public function customerBindSalerSuccess($relation_id){
        $log = pdo_get("ewei_shop_coupon_relation", array("id" => $relation_id));
        if($log){
            //$saler = m("member")->getMemberBase($log['saleropenid']);
            $saler_info = m("store")->getSalerInfo(array("openid" => $log['saleropenid']));
            $customer = m("member")->getMemberBase($log['customeropenid']);
            $params = array(
                "openid"        => $log['saleropenid'],
                "appurl"        => "pages/guide/member/index",
                "templatecode"  => "saler_bind_success",
                "datas"         => array(
                    array("name" => "导购名称", "value" => $saler_info['salername']),
                    array("name" => "客户名称", "value" => $customer['nickname']),
                    array("name" => "门店名称", "value" => $saler_info['storename']),
                )
            );
            return $this->sendNotice($params);
        }
     ...
     * 导购订单分润收入提醒
     */
    public function sendSalerBenefitMessage($orderid){
        $log = pdo_get("ewei_shop_order_benefit_log", array("orderid" => $orderid));
        if($log["saleropenid"] && $log['salerbenefit'] > 0){
            //$saler_info = m("member")->getMemberBase($log["saleropenid"]);
            $params = array(
                "openid"            => $log["saleropenid"],
                "appurl"            => "pages/guide/balance/index",
                "templatecode"      => "saler_benefit_message",
                "datas"             => array(
                    array("name" => "返佣金额", "value" => $log["salerbenefit"]),
                    array("name" => "返佣时间", "value" => date("Y-m-d H:i:s", $log["createtime"]))
                )
            );
            return $this->sendNotice($params);
     ...
     * 导购领取订单佣金提醒
     */
    public function sendSalerOrderBenefitMessage($orderid){
        $order = m("order")->getOrderDetail($orderid);
        if(!empty($order['saleropenid'])){
            $saler_info = m("store")->getSalerInfo(array("openid" => $log['saleropenid']));
            $params = array(
                "openid"            => $order['saleropenid'],
                "appurl"            => "pages/guide/order/index",
                "templatecode"      => "saler_order_benefit_notice",
                "datas"             => array(
                    array("name" => "订单金额", "value" => $order['price']),
                    array("name" => "支付方式", "value" => $order['paystr']),
                    array("name" => "门店名称", "value" => $saler_info['storename']),
     ...
     * 零售商审核通知
     */
    public function sendCommissionSuccessMessage($merchid){
        global $_W;
        $merch = pdo_get("ewei_shop_merch_user", array("id" => $merchid, "uniacid" => $_W['uniacid']));
        if($merch['status'] == 1 && $merch['accounttime'] > time()){
            $params = array(
                "openid"            => $merch['openid'],
                "appurl"            => "pages/merch/index/index",
                "templatecode"      => "commission_apply_success",
                "datas"             => array(
                    array("name" => "联系人", "value" => $merch['realname']),
                    array("name" => "申请时间", "value" => date("Y-m-d H:i:s", $merch['jointime']))
                )
            );
            return $this->sendNotice($params);
        }
     ...
     * 门店消费提醒
     */
    public function sendStoreOrderMessage($orderid){
        global $_W;
        $order = m("order")->getOrderDetail($orderid);
        ...
        if($store){
            ...
            $sql = "SELECT s.* FROM " . tablename("ewei_shop_saler") . " s " . 
                   "INNER JOIN " . tablename("ewei_shop_saler_role") . " r ON s.roleid = r.id " .
                   "WHERE s.storeid = {$store['id']} AND r.storemanager = 1";
//            $storemanager = pdo_fetchall($sql);
//            foreach($storemanager as $manager){
//                !empty($manager['openid']) ? $openids[] = $manager['openid'] : false;
//            }
            $merchid = empty($store['merchid']) ? 0 : $store['merchid'];
            if($merchid){
                $merch = pdo_get("ewei_shop_merch_user", array("id" => $merchid, "uniacid" => $_W['uniacid']));
                !empty($merch['openid']) ? $openids[] = $merch['openid'] : false;
            }
            $consume_time = empty($order['paytime']) ? $order['finishtime'] : $order['paytime'];
            $store_name = empty($order['store']) ? "" : $order['store']['storename'];
            ...
            if(!empty($openids)){
                foreach ($openids as $openid){
                    $params = array(
                        "openid"            => $openid,
                        //"appurl"            => "pages/merch/index/index",
                        "templatecode"      => "member_buy_message",
                        "datas"             => array(
                            array("name" => "支付时间", "value" => date("Y-m-d H:i:s", $consume_time)),
                            array("name" => "门店名称", "value" => $store_name),
                            array("name" => "订单金额", "value" => $order['price']),
                            array("name" => "支付方式", "value" => $order['paystr']),
                            array("name" => "赠送金额", "value" => 0),
                            array("name" => "支付时间", "value" => date("Y-m-d H:i:s", $consume_time)),
                            array("name" => "订单号", "value" => $order['ordersn']),
                        )
                    );
     ...
     * @param int $orderid
     */
    public function sendCommissionBenefitMessage($orderid){      
        $log = pdo_get("ewei_shop_order_benefit_log", array("orderid" => $orderid));
        $order = m("order")->getOrderDetail($orderid);
        if(!empty($order['addressid'])){
            $order['order_type'] = "用户线上购买";
            if(!empty($order['verifystoreid'])){
                $order['order_type'] = "用户线上购买门店核销";
            }
        } else if(!empty($order['verifystoreid']) || !empty ($order['storeid'])){
            $order['order_type'] = "用户门店购买";
        }
        $openids = array();
        if(!empty($log['storeid'])){
            $sql = "SELECT openid FROM " . tablename("ewei_shop_merch_user") . " u " .
                   "LEFT JOIN " . tablename("ewei_shop_store") . " s ON s.merchid = u.id " .
                   "WHERE s.id = {$log['storeid']}";
            $merchopenid = pdo_fetchcolumn($sql);
            if($merchopenid && $log['storebenefit'] > 0){
                $openids[] = array("openid" => $merchopenid, "type" => "merch", "benefit" => $log['storebenefit']);
            }
        }
        if(!empty($log['agentid'])){
            $agent = m("member")->getMemberBase($log['agentid']);
            if($agent && $log['agentbenefit'] > 0){
                $openids[] = array("openid" => $agent['openid'], "type" => "agent", "benefit" => $log['agentbenefit']);
            }
        }
        if(!empty($log['agent_plus'])){
            $agent = m("member")->getMemberBase($log['agent_plus']);
            if($agent && $log['agent_plus_benefit'] > 0){
                $openids[] = array("openid" => $agent['openid'], "type" => "agent", "benefit" => $log['agent_plus_benefit']);
            }
        }
        if(!empty($openids)){
            foreach ($openids as $info){
                if($info['type'] == 'merch'){
                    ...
                    $appurl = "pages/agent/index/index";
                    $remark = "用户购物运营中心分润";
                }
                $params = array(
                    "openid"            => $info['openid'],
                    "appurl"            => $appurl,
                    "templatecode"      => "commission_benefit_message",
                    "datas"             => array(
                        array("name" => "返佣金额", "value" => $info['benefit']),
                        array("name" => "订单类型", "value" => $order['order_type']),
                        array("name" => "订单号", "value" => $order['ordersn']),
                        array("name" => "返佣时间", "value" => date("Y-m-d H:i:s", $order['createtime'])),
                        array("name" => "备注信息", "value" => $remark)
                    )
                );
                $this->sendNotice($params);
            }
     ...
     * @param int $orderid
     */
    public function sendStoreVerifyMessage($orderid){
        $order = m("order")->getOrderDetail($orderid);
        $storename = empty($order['store']) ? "" : $order['store']['storename'];
        $verifyname = "";
        if(!empty($order['verifyopenid'])){
            $saler = m("store")->getSalerInfo(array("openid" =>$order['verifyopenid']));
            $saler ? $verifyname = $saler['salername'] : false;
        }
//        $openids = array();
//        $sql = "SELECT s.* FROM " . tablename("ewei_shop_saler") . " s " . 
//               "INNER JOIN " . tablename("ewei_shop_saler_role") . " r ON s.roleid = r.id " .
...
//        }
        ...
        $params = array(
            "openid"            => $order['openid'],
            "appurl"            => "pages/order/detail/index?id=" . $orderid,
            "templatecode"      => "order_verify_message",
            "datas"             => array(
                array("name" => "门店名称", "value" => $storename),
                array("name" => "核销员", "value" => $verifyname),
                array("name" => "订单号", "value" => $order['ordersn']),
                array("name" => "核销时间", "value" => date("Y-m-d H:i:s", $order['createtime']))
            )
        );
        return $this->sendNotice($params);
    }
    
    public function sendCouponMessage($openid){
        $content = "恭喜您获得了优惠券，购物可立减，分享给朋友购买可返现金。\n";
        $data = m("common")->getSysset("app");
        if(!empty($data["appid"])){
            $url = "<a href='http://www.qq.com' data-miniprogram-appid='{$data['appid']}' data-miniprogram-path='pages/sale/coupon/my/index/index'>点击跳小程序</a>";
        } else {
            $url = "";
        }
        return m("message")->sendMyTexts($openid, $content, $url);
    }
    
    public function sendChildAccountMessage($openid, $member_type = 1){
        $url = "";
        if($member_type == 1){
            $url = "merch";
        ...
        $member = m("member")->getMember($openid);
        $name = empty($member['nickname']) ? $member['realname'] : $member['nickname'];
        $params = array(
            "openid"            => $openid,
            "appurl"            => "pages_$url/$url/index/index",
            "templatecode"      => "child_account_message",
            "datas"             => array(
                array("name" => "粉丝昵称", "value" => $name),
            )
        );
        return $this->sendNotice($params);
    }
