...
class Salerrole_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		$sql = 'SELECT r.* FROM ' . 
                tablename('ewei_shop_reservation_saler_role') . '  r ' . 
    ...
    public function post(){
        ...
        if($_W['ispost']){
            ...
            if(!empty($id)){
                ...
                $result = pdo_update('ewei_shop_reservation_saler_role', $update, $params);
                ...
                $id = pdo_insert('ewei_shop_reservation_saler_role', $insert);
                if($id){
            ...
            $sql = "SELECT * FROM " . tablename('ewei_shop_reservation_saler_role') . " WHERE uniacid = :uniacid AND id = :id";
    ...
    public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,rolename FROM ' . tablename('ewei_shop_reservation_saler_role') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
        
		foreach ($items as $item) {
			pdo_update('ewei_shop_reservation_saler_role', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
			plog('reservation.salerrole.edit', '修改角色状态<br/>ID: ' . $item['id'] . '<br/>角色名称: ' . $item['rolename'] . '<br/>状态: ' . $_GPC['status'] == 1 ? '启用' : '禁用');
		}
    ...
    public function delete()
	{
		...
		foreach ($items as $item) {
			pdo_delete('ewei_shop_reservation_saler_role', array('id' => $item['id']));
			plog('reservation.salerrole.delete', '删除角色 ID: ' . $item['id'] . ' 角色名称: ' . $item['rolename'] . ' ');
		}
