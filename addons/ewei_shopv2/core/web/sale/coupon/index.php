	...
	protected function post() 
	{
		...
		if( $_W["ispost"] ) 
		{
			$data = array( 
                ...
                "autotarget" => intval($_GPC['autotarget']),
                "autonumber" => intval($_GPC['autonumber']),
                "commissiontype" => intval($_GPC['commissiontype']),
                "backcommission" => floatval($_GPC['backcommission']),
                "autogoodsid" => empty($_GPC['autogoodsid']) ? '' : implode(',', $_GPC['autogoodsid']),
                "autotype"    => intval($_GPC['autotype'])
            );
            if(empty($_GPC['autogoods'])){
                $data['autogoodsid'] = "";
            }
            
            $social_rule = $_GPC['social_rule'];
            in_array(1, $social_rule) ? $data['isnewpass']  = 1 : $data['isnewpass'] = 0;
            in_array(2, $social_rule) ? $data['finishorder'] = 1 : $data['finishorder'] = 0;
		...
		else 
		{
            ...
            $_GPC['starttime'] = $item["timestart"];
			$endtime = $item["timeend"];
            $_GPC['endtime'] = $item["timeend"];
            ...
            $autogoodsid = explode(',', $item['autogoodsid']);
            if(!empty($autogoodsid)){
                $autogoods = pdo_fetchall("SELECT id,title,thumb FROM " . tablename("ewei_shop_goods") . " WHERE uniacid = :uniacid and id in (" . $item['autogoodsid'] . ") ", array( ":uniacid" => $_W["uniacid"] ));
            }
		}
		$category = pdo_fetchall("select * from " . tablename("ewei_shop_coupon_category") . " where uniacid=:uniacid and merchid=0 order by id desc", array( ":uniacid" => $_W["uniacid"] ), "id");
