...
class Sendcoupon_EweiShopV2Page extends ComWebPage
{
	...
	public function main()
	{
		...
		$coupons = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_coupon') . (' WHERE uniacid = \'' . $_W['uniacid'] . '\' and merchid=0 ORDER BY id asc'));
        $stores = pdo_fetchall('SELECT s.id AS storeid, s.storename, s.merchid, u.merchname FROM ' . tablename('ewei_shop_store') . ' s LEFT JOIN ' . tablename('ewei_shop_merch_user') . ' u ON s.merchid = u.id ORDER BY s.merchid, s.id');
        foreach ($stores as &$store){
            $store['selectname'] = empty($store['merchid'])? '总店--' . $store['storename'] : $store['merchname'] . '--' . $store['storename'];
        }
		$hascommission = false;
	...
	public function fetch()
	{
        ...
        //按门店店员发送
            ...
            $where = '';
            if(!empty($_GPC['send_store'])){
                $where .= ' AND storeid = ' . intval($_GPC['send_store']);
            }
            $members = pdo_fetchall("SELECT openid FROM " . tablename('ewei_shop_saler') . " WHERE uniacid = {$_W['uniacid']}" . $where, array(), 'openid');
            $openids = array_keys($members);
            $plog = '发放优惠券 ID: ' . $couponid . '  方式: 门店店员  人数: ' . count($members);
        }
		else {
			if ($class1 == 7) {
				$where = '';
				...
				$openids = array_keys($members);
				$plog = '发放优惠券 ID: ' . $couponid . '  方式: 区域运营中心-' . $levelname . ' 人数: ' . count($members);
			}
		}
		...
		foreach ($members as $m) {
			...
			while ($i <= $send_total) {
                ...
                if($coupon['coupontype'] == COUPON_SOCIAL){
                    $data['sender'] = $m['openid'];
                    $data['openid'] = $m['openid'];
                    //分享批次
                    $data['sharebatch'] = md5($m['openid'] . $time);
                }
