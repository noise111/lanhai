...
class Selfexpress_EweiShopV2Page extends WebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$cityexpress = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_self_express') . ' WHERE uniacid=:uniacid AND merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => 0));
		...
		if ($_W['ispost']) {
			...
			if (!empty($cityexpress)) {
				plog('shop.selfexpress.edit', '修改同城配送 ID: ' . $cityexpress['id']);
				pdo_update('ewei_shop_self_express', $data, array('id' => $cityexpress['id']));
			}
			else {
				pdo_insert('ewei_shop_self_express', $data);
				$id = pdo_insertid();
				plog('shop.selfexpress.add', '添加同城配送 ID: ' . $id);
			}

			show_json(1, array('url' => webUrl('shop/selfexpress')));
		}
