...
//            }
			...
			if (!empty($id)) {
				...
				plog('shop.warehouse.edit', '编辑门店 ID: ' . $id);
			}
			else {
				pdo_insert('ewei_shop_selfexpress_warehouse', $data);
				$id = pdo_insertid();
				plog('shop.warehouse.add', '添加门店 ID: ' . $id);
			}
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			...
			plog('shop.warehouse.delete', '删除门店 ID: ' . $item['id'] . ' 门店名称: ' . $item['storename'] . ' ');
		}
	...
	public function displayorder()
	{
		...
		if (!empty($item)) {
			...
			plog('shop.warehouse.edit', '修改门店排序 ID: ' . $item['id'] . ' 门店名称: ' . $item['storename'] . ' 排序: ' . $displayorder . ' ');
		}
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			...
			plog('shop.warehouse.edit', '修改门店状态<br/>ID: ' . $item['id'] . '<br/>门店名称: ' . $item['storename'] . '<br/>状态: ' . $_GPC['status'] == 1 ? '启用' : '禁用');
		}
