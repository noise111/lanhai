...
class List_EweiShopV2Page extends WebPage 
{
	protected function orderData($status, $st) 
	{
		...
		if( $condition != " o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 " || !empty($sqlcondition) ) 
		{
			if( $searchfield == "member" ) 
			{
				...
				$sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress, 
                       ...
                       r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . 
                       ...
                       " left join " . tablename("ewei_shop_dispatch") . " d on d.id = o.dispatchid " . " " . $sqlcondition . 
                       " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
			}
			else 
			{
				if( $searchfield == "saler" ) 
				{
					$condition .= " AND (locate(:keyword,sm.realname)>0 or locate(:keyword,sm.mobile)>0 or locate(:keyword,sm.nickname)>0 or locate(:keyword,s.salername)>0 )";
					$sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                            d.dispatchname, r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . 
                            tablename("ewei_shop_order") . " o" . 
                            " left join " . tablename("ewei_shop_order_refund") . " r on r.id =o.refundid " . 
                            " left join " . tablename("ewei_shop_member_address") . " a on a.id=o.addressid " . 
                            " left join " . tablename("ewei_shop_dispatch") . " d on d.id = o.dispatchid " . 
                            " left join " . tablename("ewei_shop_saler") . 
                            " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid" . 
                            " left join " . tablename("ewei_shop_member") . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid" . " " . $sqlcondition . 
                            " where " . $condition . " " . $statuscondition . " ORDER BY " . $orderbuy . " DESC  ";
				}
				else 
				{
		...
		if( $_GPC["export"] == 1 ) 
		{
			...
			if( $merch_plugin ) 
			{
				$columns[] = array( "title" => "特约零售商名称", "field" => "merchname", "width" => 24 );
			}
