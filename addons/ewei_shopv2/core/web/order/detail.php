...
class Detail_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$goods = pdo_fetchall("SELECT op.specs,g.*,o.title as gtitle, o.goodssn as option_goodssn, o.productsn as option_productsn,o.qrcodeid,o.qrcodeno,
                               o.total,g.type,o.optionname,o.optionid,o.price as orderprice,o.realprice,o.changeprice,o.oldprice,o.commission1,
                               o.commission2,o.commission3,o.commissions,o.seckill,o.seckill_taskid,o.seckill_roomid" . $diyformfields . " FROM " 
                             . tablename("ewei_shop_order_goods") . " o left join " 
                             . tablename("ewei_shop_goods") . " g on o.goodsid=g.id " . " left join " 
                             . tablename("ewei_shop_goods_option") . " op on o.optionid=op.id " . 
		...
		foreach( $goods as &$r ) 
		{
            ...
            if($r["qrcodeid"] && $r["qrcodeno"]){
                $r['qrcode'] = m("qrcode")->getQrcodeUrl($r['qrcodeid'], $r['qrcodeno']);
            }
            //获取批次号信息
            $qrcode_info = pdo_get("ewei_shop_goods_qrcode", array("id" => $r['qrcodeid']));
            $length = strlen($qrcode_info['total']);
            $format = "%0".$length."d";
            $qrcode_info['batch'] = $qrcode_info['batch'] . '-' . sprintf($format, $r['qrcodeno']);
            $r['qrcode_info'] = $qrcode_info;
		}
