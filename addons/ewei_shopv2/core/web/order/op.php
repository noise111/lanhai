...
class Op_EweiShopV2Page extends WebPage 
{
	...
	public function send() 
	{
        ...
        $order_goods = pdo_fetchall("select og.id, og.goodsid, og.qrcodeid, og.qrcodeno, g.title,g.thumb,og.sendtype,g.ispresell from " . 
...
//                show_json(0, array('list3'=>$_GPC));
                ...
                if(empty($_GPC['no_qrcode'])){
                    $goods_qrcode = array();
                    foreach($_GPC as $key => $val){
                        if(stripos($key, "qrcodeid_") !== false){
                            $info = explode('_', $key);
                            $goods_qrcode[$info[1]] ? $goods_qrcode[$info[1]]['qrcodeid'] = $val : $goods_qrcode[$info[1]]= array("batch" => $val);
                        }
                        if(stripos($key, "qrcodeno_") !== false){
                            $info = explode('_', $key);
                            $goods_qrcode[$info[1]] ? $goods_qrcode[$info[1]]['qrcodeno'] = $val : $goods_qrcode[$info[1]]= array("qrcodeno" => $val);
                        }
                    }
                    if($goods_qrcode){
                        foreach($goods_qrcode as $ogoodsid => $qrcode_info){
                            $goods_info = pdo_get("ewei_shop_order_goods", array("id" => $ogoodsid));
                            $db_qr = pdo_get("ewei_shop_goods_qrcode", array("batch" => $qrcode_info['batch'], "goodsid" => $goods_info['goodsid']));
                            if($db_qr && $db_qr['total'] >= $qrcode_info['qrcodeno']){
                                $log = pdo_get("ewei_shop_goods_qrcode_log", array("qid" => $db_qr['id'], "no" => $qrcode_info['qrcodeno']));
                                if($log && empty($log['openid']) && $db_qr['id'] && $qrcode_info['qrcodeno']){
                                    pdo_update("ewei_shop_order_goods", array("qrcodeid" => $db_qr['id'], "qrcodeno" => $qrcode_info['qrcodeno'] ), array("id" => $ogoodsid));
                                    $update = array(
                                        "openid"        => $item['openid'],
                                        "updatetime"    => time()
                                    );                       
                                    ...
                                    show_json(0, "请选择商品的批次信息");
                                }
                            } else {
                                show_json(0, "商品批次信息不存在");
                            }
                        }
                    }
                }
				$time = time();
				$data = array( 
                    "sendtype" => (0 < $item["sendtype"] ? $item["sendtype"] : intval($_GPC["sendtype"])), 
        ...
        foreach($order_goods as &$ogoods){
            ...
            if($ogoods['qrcodeid'] && $ogoods['qrcodeno']){
                $batch_info = pdo_getcolumn("ewei_shop_goods_qrcode", array("id" => $ogoods['qrcodeid'], "uniacid" => $_W['uniacid']), array("batch"));
                $ogoods['batch'] = $batch_info;
            } else {
                if(empty($batch_list[$goodsid])){
                    $batches = pdo_getall("ewei_shop_goods_qrcode", array("goodsid" => $goodsid), array("id", "batch", "total"));               
                    $batch_list[$goodsid] = $batches;
                }
            }
            unset($ogoods);
        }
