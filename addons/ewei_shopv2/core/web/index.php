...
class Index_EweiShopV2Page extends WebPage
{
	public function main()
	{
        ...
        //店铺总数
        $store_total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_store') . ' s ' . ' where s.uniacid = ' . $_W["uniacid"] );
        
        //待审核店铺总数  
        $store_status0_total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_store') . ' s ' . ' where s.status = 0 and s.uniacid = ' . $_W["uniacid"] );

        //运营中心总数
        $abonus_total = pdo_fetchcolumn('select count(dm.id) from' . tablename('ewei_shop_member') . ' dm  ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_abonus_level') . ' l on l.id = dm.aagentlevel' . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isaagent =1  ');

        //待审核运营中心总数
        $abonus_status0_total = pdo_fetchcolumn('select count(dm.id) from' . tablename('ewei_shop_member') . ' dm  ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_abonus_level') . ' l on l.id = dm.aagentlevel' . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isaagent =1  and dm.aagentstatus = 0  ');
