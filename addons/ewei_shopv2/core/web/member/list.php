...
class List_EweiShopV2Page extends WebPage 
{
	...
	public function detail() 
	{
			...
			if( $hasabonus && cv("abonus.agent.check") ) 
			{
				...
				if( !empty($aadata) ) 
				{
                    ...
                    if($aagentprovinces){
                        ...
                        $aadata['agent_province_code'] = implode(',', $province_codes);
                    } else {
                        $aadata['agent_province_code'] = '';
                    }
                    ...
                    if($aagentcitys){
                        ...
                        $aadata['agent_city_code'] = implode(',', $city_codes);
                    } else {
                        $aadata['agent_city_code'] = '';
                    }
                    ...
                    if($aagentareas){
                        ...
                        $aadata['agent_area_code'] = implode(',', $area_codes);
                    } else {
                        $aadata['agent_area_code'] = '';
                    }
					...
					if( empty($_GPC["oldaagentstatus"]) && $aadata["aagentstatus"] == 1 ) 
					{
						...
						plog("abounus.agent.check", "审核运营中心 <br/>运营中心信息:  ID: " . $member["id"] . " /  " . $member["openid"] . "/" . $member["nickname"] . "/" . $member["realname"] . "/" . $member["mobile"]);
					}
					$log = "修改运营中心 <br/>运营中心信息:  ID: " . $member["id"] . " /  " . $member["openid"] . "/" . $member["nickname"];
					if( is_array($_GPC["aagentprovinces"]) ) 
					{
						$log .= "<br/>运营中心省份:" . implode(",", $_GPC["aagentprovinces"]);
					}
					if( is_array($_GPC["aagentcitys"]) ) 
					{
						$log .= "<br/>运营中心城市:" . implode(",", $_GPC["aagentcitys"]);
					}
					if( is_array($_GPC["aagentareas"]) ) 
					{
						$log .= "<br/>运营中心地区:" . implode(",", $_GPC["aagentareas"]);
					}
        ...
        //区域运营中心、绑定门店、绑定导购、来源关系
        $sql = "SELECT * FROM " . tablename("ewei_shop_coupon_relation") . " WHERE customeropenid = :openid AND uniacid = :uniacid";
        $coupon_relation = pdo_fetch($sql, array(':openid' => $member['openid'], ':uniacid' => $_W['uniacid']));
        if($coupon_relation){
            if(!empty($coupon_relation['saleropenid'])){
                $from_type = COUPON_RELATION_SALER;
                $from_str = "门店导购";
                
                $bind_store = '';
                $saler = m('store')->getSalerInfo(array('openid' => $coupon_relation['saleropenid']));
                if($saler['merchid']){
                    $merch = p('merch')->getMerch(array('id' => $saler['merchid']));
                    $bind_store .= $merch[0]['merchname'];
                }
                if($saler['storeid']){
                    $store = m('store')->getStoreInfo($saler['storeid']);
                    $bind_store .= '-' . $store['storename'];
                    if($store['areacode']){
                        //读取区域运营中心信息
                        $sql = "SELECT openid, avatar, nickname, realname FROM " . tablename("ewei_shop_member") . " WHERE uniacid = {$_W['uniacid']} " . 
                               "AND if(aagenttype=1, find_in_set('{$store['provincecode']}', agent_province_code), if(aagenttype=2, find_in_set('{$store['citycode']}', agent_city_code), find_in_set('{$store['areacode']}', agent_area_code)))";
                        $agents = pdo_fetchall($sql);
                    }
                }
                
            } else if(!empty ($coupon_relation['parentopenid'])){
                $from_type = COUPON_RELATION_COUPON;
                $from_str = "优惠券裂变";
            } else {
                $from_type = COUPON_RELATION_NATURE;
                $from_str = "自然流量";
            }          
        }
