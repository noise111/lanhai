...
//			}
			...
			if (!empty($id)) {
				...
				plog('selfexpress.route.edit', '编辑配送路线 ID: ' . $id);
			}
			else {
				pdo_insert('ewei_shop_selfexpress_route', $data);
				$id = pdo_insertid();
				plog('selfexpress.route.add', '添加配送路线 ID: ' . $id);
			}

			show_json(1, array('url' => webUrl('selfexpress.route')));
		}
...
//		print_r($item);
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			...
			plog('selfexpress.route.delete', '删除配送路线 ID: ' . $item['id'] . ' 配送路线名称: ' . $item['storename'] . ' ');
		}
	...
	public function displayorder()
	{
		...
		if (!empty($item)) {
			...
			plog('selfexpress.route.edit', '修改配送路线排序 ID: ' . $item['id'] . ' 配送路线名称: ' . $item['storename'] . ' 排序: ' . $displayorder . ' ');
		}
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			...
			plog('selfexpress.route.edit', '修改配送路线状态<br/>ID: ' . $item['id'] . '<br/>配送路线名称: ' . $item['storename'] . '<br/>状态: ' . $_GPC['status'] == 1 ? '启用' : '禁用');
		}
