...
class Saler_EweiShopV2Page extends WebPage
{
	public function main()
	{
        ...
        $sql_count = 'SELECT count(*) FROM ' . tablename('ewei_shop_saler') . " s " . (' WHERE ' . $condition);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
                ...
                //查找店员
                $sql = "SELECT * FROM " . tablename("ewei_shop_saler") . " WHERE openid = :openid AND uniacid = :uniacid";
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
            ...
            pdo_update("ewei_shop_saler", array('is_delete' => 1), array('id' => $item['id']));
