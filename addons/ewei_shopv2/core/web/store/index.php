...
class Index_EweiShopV2Page extends ComWebPage
{
	...
	public function main()
	{
		...
		$sql = 'SELECT s.*, u.merchname FROM ' . tablename('ewei_shop_store') . 's LEFT JOIN '. tablename('ewei_shop_merch_user') .' u ON u.id = s.merchid  WHERE ' . $condition . ' ORDER BY displayorder desc,id desc';
		$sql .= ' LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
        $sql_count = 'SELECT count(*) FROM ' . tablename('ewei_shop_store') . " s " . (' WHERE ' . $condition);
